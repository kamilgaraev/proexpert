...
Документ фиксирует реализованный контракт модуля ПИР для проектной документации РФ в админке ProHelper. Модуль остается в существующем контуре `DesignManagement`: BIM/IFC не вынесен отдельно, а дополняется ПД/РД, нормативными профилями, комплектностью, листами, замечаниями, нормоконтролем и выпуском комплекта.
