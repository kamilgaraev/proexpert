...
SQL;
        ...
        return [
            ...
            'legal_document_parties_counterparty_fk' => ['legal_document_parties', 'FOREIGN KEY (counterparty_id) REFERENCES counterparties (id) ON DELETE RESTRICT NOT VALID'],
...
BEGIN
    ...
    IF OLD.revoked_at IS NOT NULL OR NEW.revoked_at IS NULL
       OR (OLD.organization_id, OLD.document_id, OLD.subject_organization_id, OLD.subject_user_id,
           OLD.abilities, OLD.granted_by_user_id, OLD.expires_at, OLD.created_at)
          IS DISTINCT FROM
          (NEW.organization_id, NEW.document_id, NEW.subject_organization_id, NEW.subject_user_id,
