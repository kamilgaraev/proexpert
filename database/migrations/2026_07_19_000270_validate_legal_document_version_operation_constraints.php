    ...
    public function up(): void
    {
        ...
        foreach (LegalDocumentVersionOperationPostgresSchema::constraints() as $expected) {
            ...
            if ($actual === null || ! LegalDocumentVersionOperationPostgresSchema::constraintMatches($actual, $expected)) {
                if ($actual !== null && $expected['name'] === 'legal_archive_version_operations_state_check') {
            ...
            if (! (bool) $actual->convalidated) {
                DB::statement("ALTER TABLE {$expected['table']} VALIDATE CONSTRAINT {$expected['name']}");
            }
            $validated = $this->constraint($expected['name']);
            if ($validated === null || ! LegalDocumentVersionOperationPostgresSchema::constraintMatches($validated, $expected, true)) {
                throw new RuntimeException("legal_archive_version_operation_constraint_validation_failed:{$expected['name']}");
            }
        }
    }
