...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($this->matchesDescriptor($actual, $this->correctedBody(), $schema)) {
            return;
        }

        if (! $this->matchesDescriptor($actual, $this->currentBody(), $schema)) {
            throw new RuntimeException('legal_document_editor_save_completion_guard_descriptor_mismatch');
        }
        ...
        if (! $this->matchesDescriptor($this->functionDescriptor(), $this->correctedBody(), $schema)) {
            throw new RuntimeException('legal_document_editor_save_completion_guard_descriptor_mismatch');
        }
    ...
    {
        throw new RuntimeException('legal_document_editor_failed_save_completion_forward_only');
    }
...
SQL);
    ...
    private function correctedBody(): string
    {
        $current = $this->currentBody();
        $expected = "WHERE s.editor_session_id=NEW.editor_session_id AND s.id<>NEW.id AND s.terminal\n        AND s.save_generation < NEW.save_generation";
        $replacement = "WHERE s.editor_session_id=NEW.editor_session_id AND s.id<>NEW.id AND s.terminal\n        AND s.state IN ('reserved','processing','completed')\n        AND s.save_generation < NEW.save_generation";
        $corrected = str_replace($expected, $replacement, $current, $replacements);
        if ($replacements !== 1) {
            throw new RuntimeException('legal_document_editor_save_completion_guard_descriptor_mismatch');
        }
    ...
    private function currentBody(): string
    {
        return <<<'PLPGSQL'
BEGIN
