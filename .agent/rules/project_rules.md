...
- **Обязательно** используй `try-catch` во всех методах контроллера.
- **Логируй** все исключения через `Log::error` с контекстом для быстрой отладки.
