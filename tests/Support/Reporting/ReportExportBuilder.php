...
final class ReportExportBuilder
{
    ...
    public function runId(string $value): self
    {
        $this->runId = $value;

        return $this->mark('runId');
    }

    public function status(ReportExportStatus $value): self
    {
        $this->status = $value;
    ...
    public function exportHash(Sha256Hash $value): self
    {
        $this->exportHash = $value;

        return $this->mark('exportHash');
    }
    ...
    public function timezone(DateTimeZone $value): self
    {
        ...
        return $this->mark('timezone');
    }

    public function artifactPath(?string $value): self
    {
        $this->artifactPath = $value;

        return $this->mark('artifactPath');
    }
    ...
    public function etag(?string $value): self
    {
        ...
        return $this->mark('etag');
    }

    public function checksum(?Sha256Hash $value): self
    {
        $this->checksum = $value;

        return $this->mark('checksum');
    }
    ...
    public function queued(): ReportExport
    {
        ...
        return $this->build($this->status, null, null, null, null, null, $this->resolveQueuedPollAfterMs());
    }
    ...
    public function ready(): ReportExport
    {
        ...
        return $this->build(ReportExportStatus::READY, $artifactPath, $etag, $checksum, $sizeBytes, $rowCount, null, $readyAt);
    }
    ...
    private function build(ReportExportStatus $status, ?string $artifactPath, ?string $etag, ?Sha256Hash $checksum, ?int $sizeBytes, ?int $rowCount, ?int $pollAfterMs, ?DateTimeImmutable $readyAt = null): ReportExport
    {
        return new ReportExport($this->id, $this->runId, $status, $this->exportHash, $this->format, $this->columns, $this->sort, $this->locale, $this->timezone, $artifactPath, $etag, $checksum, $sizeBytes, $rowCount, $this->createdAt, $this->updatedAt, $readyAt, $this->expiresAt, $this->cancelRequestedAt, $this->httpDisposition, $pollAfterMs);
    }
