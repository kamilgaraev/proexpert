...
class AIAssistantRagContainerTest extends TestCase
{
    ...
    public function test_container_resolves_timeweb_rag_embedding_provider_by_default(): void
    {
        $provider = app(RagEmbeddingProviderInterface::class);

        $this->assertInstanceOf(OpenAIRagEmbeddingProvider::class, $provider);
        $this->assertSame('timeweb', $provider->provider());
    }
}
