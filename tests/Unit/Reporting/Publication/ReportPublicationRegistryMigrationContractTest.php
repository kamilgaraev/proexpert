...
final class ReportPublicationRegistryMigrationContractTest extends TestCase
{
    ...
    public function test_existing_postgres_workflow_executes_publication_gate_fail_closed(): void
    {
        ...
        self::assertStringContainsString('ReportPublicationReleaseIssuerPreflight', $releaseScript);
        self::assertStringContainsString('ProjectReportPublicationReleaseRequestRegistry', $releaseScript);
        self::assertStringContainsString("getenv('MOST_REPORT_PUBLICATION_RELEASE_TRUSTED_ROOT')", $releaseScript);
        self::assertStringNotContainsString("getenv('MOST_R15_RELEASE_TRUSTED_ROOT')", $releaseScript);
        self::assertStringContainsString("))->validate(\$options['request'], \$trustedRoot)", $releaseScript);
        self::assertStringNotContainsString('r15-candidate-manifest.json', $releaseScript);
        self::assertStringNotContainsString('r15-conformance-evidence.json', $releaseScript);
        self::assertStringNotContainsString('r15-proof-template.json', $releaseScript);
        self::assertStringNotContainsString('r15_release_request.json', $releaseScript);
