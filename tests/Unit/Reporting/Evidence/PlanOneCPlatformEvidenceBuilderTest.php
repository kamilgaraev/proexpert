...
final class PlanOneCPlatformEvidenceBuilderTest extends TestCase
{
    public function test_rejects_forged_structured_passed_ci_artifact(): void
    {
        ...
        $sourceHashes = $this->sourceHashes($root);
        $ciArtifacts = [];
        foreach (['workspace', 'saved_views', 'subscriptions', 'integration', 'fake_sequence'] as $name) {
            ...
            $ciArtifacts[$name] = [
                ...
                'command_record' => [
                    ...
                    'count' => 28,
                    'duration_ms' => 0,
                    'output_sha256' => hash('sha256', \App\BusinessModules\Core\Reporting\Support\CanonicalJson::encode($output)."\n"),
                ],
                'output' => $output,
                'published_count' => 28,
                'binding_count' => 28,
    ...
    private function platformQuality(string $root, string $commit): array
    {
        ...
        foreach ($catalog['gates'] as $gate) {
            ...
            foreach ($gate['source_paths'] as $path) {
                $sources[] = ['path' => $path, 'sha256' => hash_file('sha256', $root.'/'.$path)];
            }
            $gates[] = [
                'gate' => $gate['id'],
                ...
                'artifact_sha256' => $gate['platform_status'] === 'passed' ? str_repeat('a', 64) : null,
        ...
        return [
            ...
            'catalog' => ['path' => 'docs/reports/contracts/report-platform-gates.v1.json', 'sha256' => hash_file('sha256', $catalogPath)],
            'release_sha' => $commit,
    ...
    private function sourceHashes(string $root): array
    {
        return [
            'manifest' => hash_file('sha256', $root.'/app/BusinessModules/Core/Reporting/resources/management-catalog.v1.yaml'),
            'official_manifest' => hash_file('sha256', $root.'/app/BusinessModules/Core/Reporting/resources/official-document-catalog.v1.yaml'),
            'generated_catalog' => hash_file('sha256', $root.'/docs/reports/generated/reporting-catalog.v1.json'),
            'resource' => hash_file('sha256', $root.'/docs/reports/contracts/reporting-admin-resources.v1.schema.json'),
            'permission' => hash_file('sha256', $root.'/docs/reports/generated/report-permissions.v1.json'),
            'translation' => hash_file('sha256', $root.'/lang/ru/reports.php'),
            'route' => hash_file('sha256', $root.'/app/BusinessModules/Core/Reporting/routes.php'),
            'schema' => hash_file('sha256', $root.'/app/BusinessModules/Core/Reporting/resources/management-catalog.v1.schema.json'),
            'candidate_validation' => hash_file('sha256', $root.'/docs/reports/contracts/report-candidate-validation.schema.json'),
            'conformance_framework' => hash_file('sha256', $root.'/docs/reports/contracts/report-conformance-evidence.schema.json'),
            'publication_framework' => hash_file('sha256', $root.'/docs/reports/contracts/report-publication-ledger.schema.json'),
            'platform_quality_ledger' => hash_file('sha256', $root.'/docs/reports/contracts/report-quality-evidence.schema.json'),
        ];
    }
}
