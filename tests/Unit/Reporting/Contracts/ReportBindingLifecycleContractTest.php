...
final class ReportBindingLifecycleContractTest extends TestCase
{
    ...
    public function candidate_and_published_wrappers_are_nominally_separated(): void
    {
        ...
        $registry = new class($candidate) implements ReportDefinitionRegistry
        {
            ...
            public function manifestSha256(): \App\BusinessModules\Core\Reporting\Domain\ValueObjects\Sha256Hash
            {
                return new \App\BusinessModules\Core\Reporting\Domain\ValueObjects\Sha256Hash(str_repeat('a', 64));
            }
        };
    ...
    public function assembler_rejects_candidate_registry_before_implementation(): void
    {
        $assembler = new class implements ReportDefinitionBindingAssembler
        {
            ...
            {
                return new ReportDefinitionBindingMap([]);
            }
        };
        $registry = new class implements CandidateReportDefinitionRegistry
        {
            ...
            {
                return (new ReportDefinitionBuilder)->candidate();
            }
    ...
    #[Test]
    public function candidate_validation_result_rejects_duplicate_codes_and_preserves_input_order(): void
    {
        ...
        self::assertSame(['beta', 'alpha'], array_map(static fn (ReportCandidateValidationItem $item): string => $item->code, $result->items));
    ...
    public function binding_map_requires_matching_keys_and_missing_lookup_is_domain_not_found(): void
    {
        ...
        $beta = $this->binding('beta');
        $map = new ReportDefinitionBindingMap(['beta' => $beta, 'alpha' => $binding]);

        self::assertSame($binding, $map->get('alpha'));
        self::assertSame(['beta', 'alpha'], array_keys($map->all()));
    ...
    public function fakes_record_exact_calls_and_keep_rows_repeatable(): void
    {
        ...
        $rows = new FakeReportRowQuery($page, (static function (): \Generator {
            yield ['row_key' => 'one'];
        })());
    ...
    public function all_passing_candidate_items_produce_a_passing_result(): void
    {
        ...
        self::assertSame('beta', $result->items[0]->code);
        self::assertSame('alpha', $result->items[1]->code);
    }

    #[Test]
    public function task_eighteen_order_amendment_is_tracked_and_preserves_public_signatures(): void
    {
        $path = dirname(__DIR__, 4).'/docs/reports/contracts/plan-1a-task18-order-amendment.md';
        $amendment = file_get_contents($path);

        self::assertIsString($amendment);
        self::assertStringContainsString('forward-only', $amendment);
        self::assertStringContainsString('CandidateReportDefinitionRegistry::candidateCodes()', $amendment);
        self::assertStringContainsString('ReportDefinitionRegistry::publishedCodes()', $amendment);
        self::assertStringContainsString('Публичные constructor и method signatures обоих DTO не изменены.', $amendment);
    }
