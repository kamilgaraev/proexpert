...
final class CapturingReranker implements NormativeCandidateRerankerInterface
{
    ...
    {
        $this->intent = $workItem;
        $this->context = $context;
        $this->set = $candidateSet;
...
if ($partial) {
    ...
    if (! mkdir($staging, 0700, true)) {
        throw new RuntimeException('partial_staging_failed');
    }
    ...
    $root = $staging;
    foreach (['catalogs', 'projections', 'recordings', 'regression'] as $directory) {
        mkdir("$root/$directory", 0700, true);
    }
} else {
    if ($output !== '' && normalizePath($output) !== normalizePath($sourceRoot)) {
        builderReject('full_output_dir_invalid');
    }
    $root = $sourceRoot;
}
...
foreach ($specs as [$slug, $type, $filename, $source, $port, $intent]) {
    ...
    if (in_array($intent, ['scanned_pdf', 'dimensioned_raster'], true)) {
        foreach ($payload['evidence'] as &$evidence) {
            $evidence['locator']['coordinate_space'] = 'source_pixels_v1';
        } unset($evidence);
    ...
    if ($intent === 'engineering') {
        foreach ($payload['evidence'] as &$evidence) {
            $evidence['locator']['coordinate_space'] = 'source_units_v1';
        } unset($evidence);
    ...
    if ($port === RecordedPort::VisionExtraction) {
        writeJson("$root/recordings/$slug-source-trace.json", visionTrace($intent, $sha));
    }
    if ($type === BenchmarkSourceType::Dwg) {
        writeJson("$root/recordings/$slug-parser-proof.json", parserProof($source, $payload));
    }
...
if ($partial) {
    ...
    foreach ($sourceManifest['cases'] as $case) {
        $expected = $case['expected_locator'];
        $target = "$root/$expected";
        is_dir(dirname($target)) || mkdir(dirname($target), 0700, true);
        if (! copy("$sourceRoot/$expected", $target)) {
            throw new RuntimeException('partial_expected_copy_failed');
        }
    }
...
{
    file_put_contents($path, json_encode($data, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRESERVE_ZERO_FRACTION | JSON_THROW_ON_ERROR)."\n");
}

function isolatedOutputPath(string $path, string $sourceRoot): string
{
    $portable = str_replace('\\', '/', $path);
    if (str_contains($portable, '/../') || str_ends_with($portable, '/..')) {
        builderReject('partial_output_dir_unsafe');
    }
    if (! preg_match('#^(?:[A-Za-z]:[\\\\/]|/)#', $path)) {
        builderReject('partial_output_dir_unsafe');
    }
    $parent = realpath(dirname($path));
    if ($parent === false || is_link(dirname($path)) || pathHasLink(dirname($path))) {
        builderReject('partial_output_dir_unsafe');
    }
    $candidate = $parent.DIRECTORY_SEPARATOR.basename($path);
    $resolved = normalizePath($candidate);
    $repo = normalizePath(dirname(__DIR__, 4));
    $source = normalizePath($sourceRoot);
    if ($resolved === $source || str_starts_with($resolved, $source.'/') || $resolved === $repo || str_starts_with($resolved, $repo.'/')) {
        builderReject('partial_output_dir_unsafe');
    }
    if (file_exists($path) || is_link($path)) {
        $existing = realpath($path);
        if (is_link($path) || ! is_dir($path) || $existing === false || normalizePath($existing) !== $resolved || pathHasLink($path) || (new FilesystemIterator($path))->valid()) {
            builderReject('partial_output_dir_unsafe');
        }
    }
...
function builderReject(string $code): never
{
    fwrite(STDERR, $code.PHP_EOL);
    exit(2);
}
function pathHasLink(string $path): bool
{
    $current = $path;
...
function publishIsolatedOutput(string $staging, string $output): void
{
    if (is_dir($output) && ! rmdir($output)) {
        throw new RuntimeException('partial_output_publish_failed');
...
function refreshBaselineCatalog(string $root, RecordedFixtureCaptureBuilder $builder, array $spec): void
{
    ...
    foreach ($catalog['candidates'] as &$candidate) {
        if ($candidate['candidate_id'] === $spec['selected']) {
            $candidate['name'] = $spec['selected_name'];
        }
        if ($candidate['candidate_id'] === $spec['other']) {
            $candidate['name'] = $spec['other_name'];
        }
    }unset($candidate);
    foreach ($catalog['resources'] as &$resource) {
        if ($resource['candidate_id'] === $spec['selected']) {
            $resource['name'] = $spec['selected_name'];
        }
        if ($resource['candidate_id'] === $spec['other']) {
            $resource['name'] = $spec['other_name'];
        }
        $oldPriceId = $resource['resources']['materials'][0]['price_id'];
        $sourcePriceId = array_search($oldPriceId, $spec['price_id_map'], true);
        $sourcePriceId = $sourcePriceId === false ? $oldPriceId : $sourcePriceId;
    ...
    foreach ($catalog['prices'] as &$price) {
        ...
        $sourcePriceId = array_search($oldPriceId, $spec['price_id_map'], true);
        $sourcePriceId = $sourcePriceId === false ? $oldPriceId : $sourcePriceId;
        $price = [...$price, ...$spec['price_snapshots'][$sourcePriceId]];
        $price['id'] = $spec['price_id_map'][$sourcePriceId] ?? $oldPriceId;
    }unset($price);
    ...
    $item = $plan['local_estimates'][0]['sections'][0]['work_items'][0];
    $quantityKey = $item['metadata']['quantity_key'];
    $quantity = $quantities->get($quantityKey) ?? throw new RuntimeException('baseline quantity missing');
    $refs = array_map('strval', $quantity->evidenceIds);
    $sha = $geometry['source_sha256'];
    $context = ['organization_id' => 1, 'project_id' => 1, 'session_id' => 1, 'checkpoint_claim_token' => '018f47a2-4e5c-7d9a-8b1c-2d3e4f5a6b7c',
        ...
        'regional_context' => productionAnalysis($model->toArray(), $quantities->toArray(), $catalog)['regional_context'], 'applicability_date' => '2026-07-12'];
    $factory = app(NormativeWorkIntentFactory::class);
    $intent = $factory->intent($item, $context, $catalog['dataset_version']);
    $decision = $factory->decision($item, $context);
    $retrieval = new NormativeRetrievalService(
        new RecordedCatalogNormativeCandidateSource(RecordedBenchmarkCatalogData::fromArray($catalog)), new NormativeHardGate, 16, null);
    $set = $retrieval->retrieve($intent);
    if (count($set->candidates) !== 2) {
        throw new RuntimeException('baseline candidate set invalid');
    }
...
function visionPayload(string $intent, string $sha): array
{
    $wall = "$intent-wall-evidence";
    $room = "$intent-room-evidence";
    $locator = ['page_id' => 1, 'page_number' => 1, 'processing_unit_id' => 1, 'source_version' => "sha256:$sha", 'coordinate_space' => 'normalized_source_v1'];
    $evidence = [['key' => $wall, 'locator' => $locator]];
    $elements = [['key' => "$intent-wall", 'type' => 'wall', 'label' => null, 'polygon' => [[0.1, 0.1], [0.7, 0.1]], 'confidence' => $intent === 'freehand' ? 0.62 : 0.95, 'evidence_ref' => $wall]];
    if ($intent !== 'freehand') {
        $evidence[] = ['key' => $room, 'locator' => $locator];
        $elements[] = ['key' => "$intent-room", 'type' => 'room', 'label' => 'Комната', 'polygon' => [[0.1, 0.1], [0.7, 0.1], [0.7, 0.5], [0.1, 0.5]], 'confidence' => 0.96, 'evidence_ref' => $room];
    }

return ['schema_version' => 1, 'sheet_type' => 'floor_plan', 'evidence' => $evidence, 'elements' => $elements, 'scale_candidates' => $intent === 'freehand' ? [] : [['source' => 'dimension_text', 'meters_per_unit' => 10.0, 'confidence' => 0.99, 'evidence_ref' => $wall, 'detail' => 'visible_dimension'], ['source' => 'manual_reference', 'meters_per_unit' => 10.0, 'confidence' => 1.0, 'evidence_ref' => $room, 'detail' => 'confirmed_control_dimension']], 'warnings' => $intent === 'freehand' ? ['scale_missing'] : []];
}
function vectorPdf(): string
{
...
{
    $i = planPixels(400, 300, true);
    $s = "q 500 0 0 375 45 300 cm /Im0 Do Q\n";
...
function makePdf(array $objects): string
{
    $out = "%PDF-1.4\n";
    $offsets = [];
...
function planPixels(int $w, int $h, bool $scan): string
{
    $p = str_repeat("\xff\xff\xff", $w * $h);
    $black = function (int $x, int $y) use (&$p, $w, $h): void {
        if ($x < 0 || $y < 0 || $x >= $w || $y >= $h) {
            return;
        }$o = ($y * $w + $x) * 3;
        $p[$o] = $p[$o + 1] = $p[$o + 2] = "\0";
    ...
    for ($t = 0; $t < 5; $t++) {
        ...
        $black($x, 40);
        $black($x, 260);
    }drawBitmapText($black, $scan ? 130 : 120, 270, '8.0 m');
    drawBitmapText($black, 300, 125, '5.5 m');

    return $p;
}
function bitmapFont(): array
{
    return ['8' => ['111', '101', '111', '101', '111'], '5' => ['111', '100', '111', '001', '111'], '0' => ['111', '101', '101', '101', '111'], '.' => ['0', '0', '0', '0', '1'], 'm' => ['00000', '11011', '10101', '10101', '10101'], ' ' => ['0', '0', '0', '0', '0']];
}
function bitmapPoints(string $text): array
{
    $points = [];
    $cursor = 0;
    foreach (str_split($text) as $char) {
        $rows = bitmapFont()[$char];
        $width = strlen($rows[0]);
        foreach ($rows as $y => $row) {
            foreach (str_split($row) as $x => $on) {
                if ($on === '1') {
                    $points[] = [$cursor + $x, $y];
                }
            }
...
function drawBitmapText(Closure $black, int $x, int $y, string $text): void
{
    foreach (bitmapPoints($text) as [$dx,$dy]) {
        for ($sy = 0; $sy < 2; $sy++) {
            for ($sx = 0; $sx < 2; $sx++) {
                $black($x + $dx * 2 + $sx, $y + $dy * 2 + $sy);
            }
        }
    }
}
function freehand(): string
{
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 400"><title>Неуверенный эскиз</title><path id="uncertain-outline" d="M70 90 L510 77 L525 330 L82 345 Z" fill="none" stroke="black" stroke-width="7"/><path id="uncertain-divider" d="M80 210 Q260 180 520 220" fill="none" stroke="black" stroke-width="5"/><path id="freehand-opening" d="M265 82 L330 80" fill="none" stroke="red" stroke-width="9"/><text id="review-question" x="245" y="375" font-size="26">? размер</text></svg>';
}
function engineering(): string
{
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 500"><title>Инженерный план</title><rect id="room-outline" x="70" y="60" width="650" height="370" fill="none" stroke="black" stroke-width="4"/><path id="door-opening" d="M350 60 h90" stroke="white" stroke-width="10"/><line id="riser-110" x1="180" y1="80" x2="180" y2="410" stroke="blue" stroke-width="8"/><circle id="riser-node" cx="180" cy="245" r="24" fill="none" stroke="blue" stroke-width="6"/><text id="riser-label" x="215" y="250" font-size="26">Стояк 110</text><text id="dimension-width" x="300" y="470" font-size="22">6500 mm</text><text id="dimension-height" x="730" y="250" font-size="22">3700 mm</text></svg>';
}
function dwg(): string
{
    $b = @file_get_contents(dirname(__DIR__).'/Vision/simple-house.dwg');
    if (is_string($b)) {
        return $b;
    }throw new RuntimeException('dwg fixture missing');
}
function runCapture(array $command): array
{
    $json = shell_exec(implode(' ', array_map('escapeshellarg', $command)));
    if (! is_string($json) || $json === '') {
        throw new RuntimeException('geometry capture failed');
    }

return json_decode($json, true, 128, JSON_THROW_ON_ERROR);
}
function captureVectorPdf(string $path): array
{
    return runCapture(['python', dirname(__DIR__, 4).'/app/BusinessModules/Addons/EstimateGeneration/bin/pdf_geometry_extract.py', '--input', $path, '--workspace', dirname($path), '--contract-vector']);
}
function captureDwg(string $path): array
{
    $binary = getenv('LIBREDWG_DWGREAD_BINARY') ?: getenv('USERPROFILE').'/.cache/most-libredwg/0.13.4/win64/dwgread.exe';
    $workspace = sys_get_temp_dir().DIRECTORY_SEPARATOR.'most-replay-dwg-'.bin2hex(random_bytes(8));
    if (! mkdir($workspace, 0700, true)) {
        throw new RuntimeException('dwg_workspace_failed');
    }$input = $workspace.DIRECTORY_SEPARATOR.'input.dwg';
    if (! copy($path, $input)) {
        removeTree($workspace);
        throw new RuntimeException('dwg_workspace_failed');
    }try {
        return runCapture(['python', dirname(__DIR__, 4).'/app/BusinessModules/Addons/EstimateGeneration/bin/cad_geometry_extract.py', '--input', $input, '--workspace', $workspace, '--dwgread', $binary]);
    } finally {
        removeTree($workspace);
    }
}
function parserProof(string $source, array $payload): array
{
    ...
    return ['schema_version' => 1, 'source_sha256' => hash('sha256', $source), 'runtime_version' => $payload['runtime_version'], 'canonical_output_sha256' => hash('sha256', $canonical), 'entity_count' => count($payload['entities']), 'text_count' => count($payload['texts']), 'dimension_count' => count($payload['dimensions'])];
}
function visionFormat(string $intent): string
{
    return match ($intent) {
        'scanned_pdf' => 'raster_pdf','dimensioned_raster' => 'ppm',default => 'svg'
    };
}
function visionTrace(string $intent, string $sha): array
{
    if (in_array($intent, ['scanned_pdf', 'dimensioned_raster'], true)) {
        return ['source_sha256' => $sha, 'labels' => [['bbox' => [$intent === 'scanned_pdf' ? 130 : 120, 270, 36, 10], 'text' => '8.0 m'], ['bbox' => [300, 125, 36, 10], 'text' => '5.5 m']], 'room_polygon' => [[40, 40], [360, 40], [360, 260], [40, 260]], 'meters_per_unit' => 0.025];
    }
    if ($intent === 'engineering') {
...
function productionGeometry(array $payload, RecordedPort $port, ?array $confirmationPayload = null): array
{
    ...
    $refs = [];
    if ($vision !== null) {
        foreach ($vision->evidence as $row) {
            $refs[] = $row->key;
        }
    }
    if ($vector !== null) {
        foreach ($vector->entities as $row) {
            $refs[] = 'vector:'.$row['handle'];
        }
    }
    $confirmation = $confirmationPayload !== null ? GeometryConfirmationData::fromArray($confirmationPayload) : null;
    if ($confirmation !== null) {
        foreach ($confirmation->scaleEvidence as $item) {
            $refs[] = ($item['role'] === 'measured_segment' ? 'vector:' : 'confirmation:').($item['value_handle'] ?? $item['entity_handle']);
    ...
    if ($assembled->clarifications !== [] || ($assembled->model->metrics['complete'] ?? false) !== true) {
        throw new RuntimeException('generated geometry incomplete');
    }
...
function authoredCatalog(array $spec): array
{
    ...
    $resources = [];
    $prices = [];
    foreach ($spec['candidates'] as $row) {
        [$id,$norm,$code,$name,$material,$technology,$structure,$lexical,$semantic,$priceId,$resourceCode,$resourceName,$resourceUnit,$resourceQuantity,$price] = $row;
