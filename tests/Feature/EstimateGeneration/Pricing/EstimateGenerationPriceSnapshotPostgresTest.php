...
final class EstimateGenerationPriceSnapshotPostgresTest extends EstimateGenerationPostgresTestCase
{
    ...
    public function test_manual_quantity_confirmation_persists_scoped_user_evidence_and_rolls_back_atomically(): void
    {
        ...
        $draft['regional_context'] = [
            ...
            'period_id' => $fixture['period_id'], 'estimate_regional_price_version_id' => $fixture['version_id'],
        ];
        ...
        $inputVersion = $fixture['input_version'];
        $draft['source_input_version'] = $inputVersion;
        ...
        self::assertNull(app(AcceptedQuantityEvidenceVerifier::class)->rejectionReason(
        ...
        self::assertSame('calculated', $regeneratedItem['pricing_status'] ?? null, json_encode($regeneratedItem, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT));
        self::assertNull($regeneratedItem['pricing_blocker'] ?? null);
        self::assertNotNull(app(AuthoritativePackagePricingGuard::class)->inputs(
            (int) $session->organization_id,
            (int) $session->project_id,
            (int) $session->id,
            $inputVersion,
        ...
        self::assertNotNull(
            $finalized->pricing_finalized_at,
            json_encode((array) $finalized, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT),
        );
    ...
    public function test_database_builds_deterministic_price_snapshot_and_protects_every_trust_input(): void
    {
        ...
        try {
            ...
            self::assertMatchesRegularExpression('/^sha256:[a-f0-9]{64}$/', $snapshot['source_reference']);
            ...
            app(EstimateGenerationPackagePersistenceService::class)->syncFromDraft(
                $session,
                $this->serviceDraft($fixture, '2.5'),
                $fixture['input_version'],
            );
    ...
    public function test_database_semantic_project_material_price_matches_cross_dataset_median_and_regional_priority(): void
    {
        ...
        try {
            ...
            $finalize = function (string $logicalKey, int $priceId, array $selection) use (&$fixture, $ruleId): object {
                $evidence = $this->evidenceForLogicalKey($fixture, $logicalKey);
                $itemId = DB::table('estimate_generation_package_items')->insertGetId(array_replace(
                    $this->itemPayload($fixture, $logicalKey, 1, null),
                    [
                        'quantity_evidence_id' => $evidence['id'],
                        'quantity_evidence_fingerprint' => $evidence['fingerprint'],
                    ],
                ));
            ...
            $regionalVersion = strtolower((string) str()->ulid());
            $regionalDatasetId = DB::table('estimate_dataset_versions')->insertGetId([
                'source_type' => 'fgis_labor_prices',
                'version_key' => $regionalVersion,
                'bucket' => 'contract',
                'prefix' => $regionalVersion,
                'status' => 'parsed',
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            $regionalVersionId = DB::table('estimate_regional_price_versions')->insertGetId([
                'source' => 'fgiscs',
                'region_id' => $fixture['region_id'],
                'price_zone_id' => $fixture['zone_id'],
            ...
            $regionalLaborPriceId = DB::table('estimate_resource_prices')->insertGetId([
                'dataset_version_id' => $regionalDatasetId,
                'regional_price_version_id' => $regionalVersionId,
                'region_id' => $fixture['region_id'],
                'price_zone_id' => $fixture['zone_id'],
                'period_id' => $fixture['period_id'],
                'resource_code' => $fixture['resource_code'],
            ...
            DB::table('estimate_regional_price_versions')->where('id', $regionalVersionId)->update(['status' => 'active']);
            $fixture = array_replace($fixture, [
                'version_id' => $regionalVersionId,
                'price_id' => $regionalLaborPriceId,
            ]);
    ...
    {
        return [
            'source_input_version' => $f['input_version'],
            'local_estimates' => [
                [
                    'key' => 'service-package',
                        ...
                        [
                            'work_items' => [
                                [
                                    ...
                                    'name' => 'Трудозатраты рабочих',
                                    'unit' => 'h',
                                    'quantity' => $quantity,
                                    'quantity_evidence_id' => $f['evidence_id'],
                                    'quantity_evidence_fingerprint' => $f['fingerprint'],
                                    'normative_match' => ['status' => 'matched', 'norm_id' => $f['norm_id']],
                                    'labor' => [[
                                        'normative_ref' => [
                                            'norm_resource_id' => $f['norm_resource_id'],
                                            'price_id' => $f['price_id'],
                                            'unit_conversion_id' => $f['conversion_id'],
                                        ],
                                    ]],
                                    'price_snapshot' => [
    ...
    private function fixture(): array
    {
        ...
        $inputVersion = PipelineBaseInputVersion::fromProjection([], []);
        ...
        $normId = DB::table('estimate_norms')->insertGetId(['collection_id' => $collectionId, 'code' => '01-01-001-01', 'name' => 'Трудозатраты рабочих', 'unit' => 'h', 'created_at' => $now, 'updated_at' => $now]);
        ...
        $evidenceId = DB::table('estimate_generation_evidence')->insertGetId(['organization_id' => $organizationId, 'project_id' => $projectId, 'session_id' => $sessionId, 'type' => 'work_item', 'source_type' => 'user_input', 'source_ref' => 'input:1', 'source_version' => $inputVersion, 'locator' => json_encode(['item_key' => 'item:'.hash('sha256', 'item-1')], JSON_THROW_ON_ERROR), 'value' => json_encode(['work_code' => 'work_type:1', 'quantity' => '2.5', 'unit' => 'h'], JSON_THROW_ON_ERROR), 'confidence' => 1, 'producer_name' => 'contract', 'producer_version' => 'contract:abcdef', 'fingerprint' => $fingerprint, 'created_at' => $now, 'updated_at' => $now]);
        $resourceCode = $suffix;
        $datasetVersionKey = $suffix;
        $fixture = compact('sessionId', 'packageId', 'datasetId', 'priceDatasetId', 'datasetVersionKey', 'collectionId', 'resourceCode', 'normId', 'manualNormId', 'normResourceId', 'regionId', 'zoneId', 'periodId', 'versionId', 'priceId', 'conversionId', 'evidenceId', 'fingerprint', 'inputVersion');
    ...
    /** @return array{id:int,fingerprint:string} */
    private function evidenceForLogicalKey(array $fixture, string $logicalKey): array
    {
        $now = now();
        $fingerprint = hash('sha256', 'quantity-evidence-'.$logicalKey);
        $id = DB::table('estimate_generation_evidence')->insertGetId([
            'organization_id' => DB::table('estimate_generation_evidence')
                ->where('id', $fixture['evidence_id'])->value('organization_id'),
            'project_id' => DB::table('estimate_generation_evidence')
                ->where('id', $fixture['evidence_id'])->value('project_id'),
            'session_id' => $fixture['session_id'],
            'type' => 'work_item',
            'source_type' => 'user_input',
            'source_ref' => sprintf('input:%u', crc32($logicalKey)),
            'source_version' => 'contract:abcdef',
            'locator' => json_encode(['item_key' => 'item:'.hash('sha256', $logicalKey)], JSON_THROW_ON_ERROR),
            'value' => json_encode([
                'work_code' => 'work_type:'.hash('sha256', $logicalKey),
                'quantity' => '2.5',
                'unit' => 'h',
            ], JSON_THROW_ON_ERROR),
            'confidence' => 1,
            'producer_name' => 'contract',
            'producer_version' => 'contract:abcdef',
