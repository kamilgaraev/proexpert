...
final class EstimateGenerationFailureLedgerPostgresTest extends TestCase
{
    ...
    public function committed_winner_unblocks_duplicate_event_without_double_occurrence(): void
    {
        ...
        try {
            ...
            self::assertSame(1, DB::table('estimate_generation_failure_occurrences')->where('event_id', $eventId)->count());
        } finally {
    ...
    public function postgres_enforces_tenant_privacy_collision_resolution_and_immutability(): void
    {
        ...
        try {
            ...
            $this->assertRejected(fn () => DB::table('estimate_generation_failures')->insert(['id' => (string) Str::uuid()]));
            $this->assertRejected(fn () => DB::table('estimate_generation_failures')->where('fingerprint', $first->fingerprint)->update(['code' => 'changed']));
            $this->assertRejected(fn () => DB::table('estimate_generation_failures')->where('fingerprint', $first->fingerprint)->delete());
            $this->assertRejected(fn () => DB::selectOne('SELECT record_estimate_generation_failure(?::jsonb)', [json_encode([
                'id' => (string) Str::uuid(), 'fingerprint' => $first->fingerprint,
                'organization_id' => $fixture['organization_id'] + 999999, 'project_id' => $fixture['project_id'],
                'session_id' => $fixture['session_id'], 'correlation_id' => (string) Str::uuid(),
                'stage' => 'understand_documents', 'operation' => 'process_unit', 'category' => 'terminal',
                'code' => 'unexpected_internal_failure', 'attempt' => 1, 'safe_context' => ['prompt' => 'secret'],
                'seen_at' => '2026-07-11T10:00:00+00:00',
            ], JSON_THROW_ON_ERROR)]));
            ...
            self::assertSame(0, DB::table('estimate_generation_failure_occurrences')->where('fingerprint', $first->fingerprint)->count());
        } finally {
    ...
    private function assertRejected(callable $operation): void
    {
        ...
        try {
            $operation();
            self::fail('Invalid failure ledger mutation was accepted.');
