...
final class ProcessingUnitTenantPostgresContractTest extends EstimateGenerationPostgresTestCase
{
    #[Test]
    public function composite_scope_null_on_delete_and_session_cascade_are_enforced_together(): void
    {
        ...
        $this->assertRejected(fn () => DB::table('estimate_generation_processing_units')->insert([
            ...$this->unitRow($a, true), 'organization_id' => $b['organization_id'],
        ]));
        $this->assertRejected(fn () => DB::table('estimate_generation_processing_units')->insert([
            ...$this->unitRow($a, true), 'project_id' => $b['project_id'],
        ]));
        $this->assertRejected(fn () => DB::table('estimate_generation_processing_units')->insert([
            ...$this->unitRow($a, true), 'session_id' => $b['session_id'],
        ]));
        $this->assertRejected(fn () => DB::table('estimate_generation_processing_units')->insert([
            ...$this->unitRow($a, true), 'document_id' => $b['document_id'],
        ]));
    ...
    private function unitRow(array $fixture, bool $databaseJson = false): array
    {
        $locator = [
            'source_kind' => 'pdf',
            'source_version' => 'contract-v1',
            'coordinate_space' => 'pdf_page_pixels',
            'artifact_path' => sprintf('org-%d/estimate-generation/contract.pdf', $fixture['organization_id']),
            'artifact_sha256' => 'sha256:'.str_repeat('a', 64),
            'artifact_version_id' => 'sha256:'.str_repeat('a', 64),
        ];
        ...
        return ['organization_id' => $fixture['organization_id'], 'project_id' => $fixture['project_id'],
            ...
            'source_version' => 'contract-v1', 'status' => 'pending',
            'locator' => $databaseJson ? json_encode($locator, JSON_THROW_ON_ERROR) : $locator,
            'metadata' => $databaseJson ? '{}' : (object) []];
    }
