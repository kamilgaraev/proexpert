...
{
    private function assertDeclineCannotOverwriteAcceptedInvitation(): void
    {
        $ownerOrganization = Organization::factory()->create();
        $owner = $this->organizationUser($ownerOrganization, 'owner-decline-race@example.test');
        ...
        $invitation = ProjectParticipantInvitation::query()->create([
            ...
            'organization_name' => 'Accept versus decline observer',
        ]);
        $organization = Organization::factory()->create();
        $user = $this->organizationUser($organization, 'accept-decline-race@example.test');

        $suffix = bin2hex(random_bytes(5));
        $lockKey = random_int(100000, 2000000000);
        $function = "invitation_decline_barrier_{$suffix}";
        $trigger = "invitation_decline_barrier_{$suffix}";
        $processes = [];
        ...
        try {
            ...
            $processes[] = $this->startWorker($invitation->token, $user->id, $organization->id);
            $processes[] = $this->startDeclineWorker($invitation->token);
            ...
            self::assertSame(409, $results[1]['code']);
            self::assertSame(ProjectParticipantInvitation::STATUS_ACCEPTED, $invitation->fresh()->status);
            self::assertSame((int) $organization->id, (int) $invitation->fresh()->accepted_organization_id_snapshot);
        } finally {
            if ($advisoryLocked) {
                DB::select('SELECT pg_advisory_unlock(?)', [$lockKey]);
            }
    ...
    public function test_two_organizations_cannot_accept_one_invitation_concurrently(): void
    {
        ...
        $this->assertDeclineCannotOverwriteAcceptedInvitation();
    }
    ...
    /** @return array{process: resource, pipes: array<int, resource>, backend_pid: int} */
    private function startDeclineWorker(string $token): array
    {
        $environment = getenv();
        $environment = is_array($environment) ? $environment : [];
        $connection = (array) config('database.connections.'.config('database.default'));
        $environment = array_merge($environment, [
            ...
            'DB_USERNAME' => (string) $connection['username'],
            'DB_PASSWORD' => (string) $connection['password'],
            'MOST_INVITATION_DECLINE_TOKEN' => $token,
        ]);
        $pipes = [];
        $process = proc_open(
            [PHP_BINARY, base_path('tests/Support/Auth/invitation_decline_worker.php')],
            [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']],
            $pipes,
            base_path(),
            $environment,
        ...
        if (! is_resource($process)) {
            throw new RuntimeException('Unable to start invitation decline worker.');
        }
        ...
        if (! is_string($ready) || preg_match('/^READY:([1-9][0-9]*)\R$/D', $ready, $matches) !== 1) {
            throw new RuntimeException('Invitation decline worker did not become ready: '.stream_get_contents($pipes[2]));
        }
