...
final class EstimateGenerationStatusMutationBoundaryTest extends TestCase
{
    ...
    private const ALLOWED_MUTATORS = [
        ...
        'Application/Sessions/CreateEstimateGenerationSession.php',
    ];

    #[Test]
    public function session_status_and_version_are_mutated_only_by_the_workflow_store_or_explicit_factory(): void
    {
        self::assertSame([], $this->violations());
    }

    #[Test]
    public function initial_factory_can_only_insert_the_immutable_draft_state(): void
    {
        ...
        self::assertSame(1, substr_count($source, '->create(['));
        self::assertStringContainsString("'status' => EstimateGenerationStatus::Draft->value", $source);
        self::assertStringContainsString("'state_version' => 0", $source);
        self::assertStringContainsString("'resume_status' => null", $source);
    ...
    private function fileViolations(string $path): array
    {
        ...
        foreach ($finder->findInstanceOf($nodes, Node\Expr\Assign::class) as $assignment) {
            if ($assignment->var instanceof Node\Expr\PropertyFetch
                ...
                && $this->isManagedField($assignment->var->name)) {
        ...
        foreach ($finder->findInstanceOf($nodes, Node\Expr\MethodCall::class) as $call) {
            if (! $this->isSessionReceiver($call->var, $sessionVariables)
                ...
                || ! in_array($call->name->toString(), ['fill', 'forceFill', 'update', 'updateOrCreate', 'upsert'], true)) {
                continue;
            }

            if ($this->argumentsMutateManagedFields($call->args)) {
    ...
    private function argumentsMutateManagedFields(array $arguments): bool
    {
        foreach ($arguments as $argument) {
            ...
            foreach ($argument->value->items as $item) {
                if ($item?->key instanceof Node\Scalar\String_
                    && in_array($item->key->value, ['status', 'state_version', 'resume_status'], true)) {
                    return true;
                }
            }
        }
    ...
    private function isManagedField(Node\Identifier|Node\Expr $name): bool
    {
        return $name instanceof Node\Identifier
            && in_array($name->toString(), ['status', 'state_version', 'resume_status'], true);
    }
}
