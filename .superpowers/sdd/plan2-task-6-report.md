...
- Document manifest job получает persisted checkpoint claim только после блокировки и проверки session/document/source snapshot. Детектор работает вне транзакции, а запись units/status допускается только через `DocumentManifestPublicationFence`, повторно проверяющий active token/lease, session version/status и source lineage под блокировками. Unit rows служат durable outbox: внешний dispatch не выполняется внутри publication-транзакции, плановый recovery worker выбирает только актуальные document/source units.
...
- Повторный DB-less gate после исправления: `166 passed (996 assertions)`; PHPStan: `No errors`.
