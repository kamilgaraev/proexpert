...
final readonly class TimewebVisionProvider implements VisionProvider
{
    ...
    private static function arbitrationSystemPrompt(): string
    {
        return implode("\n", [
            ...
            'Each decision intent contains only claim_id, status, supporting_claim_ids, evidence_refs and a natural Unicode reason.',
            'status is accepted, candidate or unresolved. Use only claim and evidence identifiers from the supplied allowlist.',
            'For unresolved, preserve the conflicting claim and evidence as an unresolved decision. Do not generate user questions or answer choices.',
    ...
    private static function systemPrompt(
        ...
        return implode("\n", [
            ...
            "The supplied role {$sheetRole} is only a preliminary hint. Classify the full page yourself and return the best supported role with 0..{$maxFacts} evidence-backed facts; unknown may contain generic facts and typed unknown values.",
            ...
            'value has exactly type and data. type is exactly number, string, boolean, enum or unknown. Number data is a canonical plain decimal string with at most four fractional digits and absolute value at most 1000000000000, without exponent, leading plus, leading zero or negative zero. For unknown, data and unit must both be null; this is required whenever the document does not explicitly state a fact. For known values, data must match its declared type and unit is null or a visible unit string.',
            ...
            'For plan inspect rooms, walls, openings, axes, dimensions, areas, levels, materials/finishes, engineering elements, notes and cross-sheet references. Preserve every visible sanitary fixture, kitchen fixture/equipment and furniture object as a separate evidence-backed fact even when it is conditional or unsuitable for automatic estimation. Use sanitary_fixture for toilets, washbasins, baths and showers; kitchen_fixture for sinks and cooktops; furniture for beds, tables, chairs, sofas and cabinets; equipment for other identified appliances; unknown_fixture when the type is not proven. Put a visible count in the string value when supported and keep the room in entityKey. Never turn a visual object into a normative or priced item. For section inspect elevations, levels, structure, openings, roof, dimensions, materials and engineering elements. For explication/specification inspect every bounded table row, quantity, material, equipment, mark, note and reference.',
            'Use technology_candidate, assumption and risk only as non-authoritative candidates. If a required material or parameter is not explicit, return an unknown value and an evidence-backed limitation; never convert it to a confirmed known value.',
            ...
            'Use one stable entityKey for the same explicitly marked entity across plan, facade, section, explication and specification references. One entity may have multiple factType entries; every (entityKey, factType) pair is unique. If identity is ambiguous, keep separate candidates and preserve the ambiguity as an unknown value instead of merging them.',
