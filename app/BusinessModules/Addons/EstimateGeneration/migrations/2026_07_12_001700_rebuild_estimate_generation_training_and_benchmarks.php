...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            foreach ([
                'dataset_key' => static fn (Blueprint $table) => $table->uuid('dataset_key')->nullable(),
                'version' => static fn (Blueprint $table) => $table->unsignedInteger('version')->nullable(),
                'dataset_type' => static fn (Blueprint $table) => $table->string('dataset_type', 20)->nullable(),
                'scope' => static fn (Blueprint $table) => $table->string('scope', 20)->nullable(),
                'approved_by' => static fn (Blueprint $table) => $table->unsignedBigInteger('approved_by')->nullable(),
                'approved_at' => static fn (Blueprint $table) => $table->timestampTz('approved_at')->nullable(),
            ] as $column => $add) {
            ...
            foreach ([
                ...
                'reviewed_at' => static fn (Blueprint $table) => $table->timestampTz('reviewed_at')->nullable(),
            ] as $column => $add) {
            ...
            DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION eg_training_online_write_fence() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  NEW.dataset_key := COALESCE(NEW.dataset_key, NEW.uuid);
  NEW.version := COALESCE(NEW.version, 1);
  NEW.dataset_type := COALESCE(NEW.dataset_type, 'development');
  NEW.scope := COALESCE(NEW.scope, 'organization');
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS eg_training_online_write_fence ON estimate_generation_training_datasets;
CREATE TRIGGER eg_training_online_write_fence BEFORE INSERT OR UPDATE ON estimate_generation_training_datasets FOR EACH ROW EXECUTE FUNCTION eg_training_online_write_fence();

CREATE OR REPLACE FUNCTION eg_training_example_review_write_fence() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.status IN ('accepted', 'indexed') AND (NEW.reviewed_by IS NULL OR NEW.reviewed_at IS NULL) THEN
    NEW.status := 'pending';
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS eg_training_example_review_write_fence ON estimate_generation_training_examples;
CREATE TRIGGER eg_training_example_review_write_fence BEFORE INSERT OR UPDATE ON estimate_generation_training_examples FOR EACH ROW EXECUTE FUNCTION eg_training_example_review_write_fence();
SQL);
            $runtime->checkpoint('001700_structure');

            $runtime->backfillDatasets();
            $runtime->backfillExamples();
            $runtime->checkpoint('001700_backfill');
            ...
            foreach ([
                ...
                'eg_training_dataset_status_chk' => "CHECK (status IN ('draft','processing','review_required','approved','rejected','archived'))",
                'eg_training_dataset_scope_chk' => "CHECK (scope = 'organization' AND organization_id IS NOT NULL)",
                'eg_training_dataset_approval_chk' => "CHECK (status <> 'approved' OR (approved_by IS NOT NULL AND approved_at IS NOT NULL))",
                'estimate_generation_training_datasets_approved_by_foreign' => 'FOREIGN KEY (approved_by) REFERENCES system_admins(id) ON DELETE SET NULL',
            ] as $name => $definition) {
                $runtime->ensureConstraint('estimate_generation_training_datasets', $name, $definition);
                $runtime->validateConstraint('estimate_generation_training_datasets', $name);
            }
            ...
            $runtime->checkpoint('001700_indexes');
            foreach ([
                'eg_training_example_review_chk' => "CHECK (status NOT IN ('accepted','indexed') OR (reviewed_by IS NOT NULL AND reviewed_at IS NOT NULL))",
                'estimate_generation_training_examples_reviewed_by_foreign' => 'FOREIGN KEY (reviewed_by) REFERENCES system_admins(id) ON DELETE RESTRICT',
            ] as $name => $definition) {
                $runtime->ensureConstraint('estimate_generation_training_examples', $name, $definition);
                $runtime->validateConstraint('estimate_generation_training_examples', $name);
            }
            ...
            foreach ([
                'eg_benchmark_status_chk' => "CHECK (status IN ('running','completed','failed'))",
                'eg_benchmark_terminal_chk' => "CHECK ((status = 'running' AND completed_at IS NULL) OR (status IN ('completed','failed') AND completed_at IS NOT NULL))",
                'eg_benchmark_results_chk' => 'CHECK (NOT (case_results IS NOT NULL AND case_results_storage_path IS NOT NULL))',
                'eg_benchmark_s3_chk' => "CHECK (case_results_storage_path IS NULL OR case_results_storage_disk = 's3')",
                'eg_benchmark_json_bounds_chk' => 'CHECK (pg_column_size(model_versions) <= 1048576 AND (metrics IS NULL OR pg_column_size(metrics) <= 1048576) AND (case_results IS NULL OR pg_column_size(case_results) <= 1048576))',
                'eg_benchmark_cost_chk' => "CHECK (cost_amount >= 0 AND currency ~ '^[A-Z]{3}$')",
                'eg_benchmark_dataset_version_fk' => 'FOREIGN KEY (training_dataset_id, dataset_version) REFERENCES estimate_generation_training_datasets(id, version) ON DELETE RESTRICT',
            ] as $name => $definition) {
                $runtime->ensureConstraint('estimate_generation_benchmark_runs', $name, $definition);
                $runtime->validateConstraint('estimate_generation_benchmark_runs', $name);
            }
            $runtime->checkpoint('001700_constraints');
...
CREATE OR REPLACE FUNCTION eg_guard_training_example_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS eg_training_example_immutable ON estimate_generation_training_examples;
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS eg_benchmark_dataset_scope ON estimate_generation_benchmark_runs;
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS eg_benchmark_run_immutable ON estimate_generation_benchmark_runs;
