...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        try {
            DB::statement('ALTER TABLE public.estimate_generation_failure_events DROP CONSTRAINT IF EXISTS eg_failure_events_safe_context_closed_ck_v3');
            DB::statement('ALTER TABLE public.estimate_generation_failure_events DROP CONSTRAINT IF EXISTS eg_failure_events_safe_context_values_ck_v3');
            DB::statement('ALTER TABLE public.estimate_generation_failure_events DROP CONSTRAINT IF EXISTS eg_failure_events_safe_context_prompt_ck');

            DB::statement(<<<'SQL'
            ALTER TABLE public.estimate_generation_failure_events
            ADD CONSTRAINT eg_failure_events_safe_context_closed_ck_v3 CHECK (
        ...
        SQL);
            DB::unprepared(<<<'SQL'
            ALTER TABLE public.estimate_generation_failure_events
            ADD CONSTRAINT eg_failure_events_safe_context_values_ck_v3 CHECK (
        ...
        SQL);
            DB::statement(<<<'SQL'
            ALTER TABLE public.estimate_generation_failure_events
            ADD CONSTRAINT eg_failure_events_safe_context_prompt_ck CHECK (
                NOT jsonb_path_exists(
                    safe_context,
                    '$.* ? (@.type() == "string" && @ like_regex "prompt" flag "i")'
                )
            ) NOT VALID
        SQL);

            DB::statement('ALTER TABLE public.estimate_generation_failure_events VALIDATE CONSTRAINT eg_failure_events_safe_context_closed_ck_v3');
            DB::statement('ALTER TABLE public.estimate_generation_failure_events VALIDATE CONSTRAINT eg_failure_events_safe_context_values_ck_v3');
            DB::statement('ALTER TABLE public.estimate_generation_failure_events VALIDATE CONSTRAINT eg_failure_events_safe_context_prompt_ck');
            ...
            DB::transaction(function (): void {
                ...
                ALTER TABLE public.estimate_generation_failure_events
                    RENAME CONSTRAINT eg_failure_events_safe_context_closed_ck_v3 TO eg_failure_events_safe_context_closed_ck;
                ALTER TABLE public.estimate_generation_failure_events
                    RENAME CONSTRAINT eg_failure_events_safe_context_values_ck_v3 TO eg_failure_events_safe_context_values_ck;
                SQL);
            });
        } finally {
