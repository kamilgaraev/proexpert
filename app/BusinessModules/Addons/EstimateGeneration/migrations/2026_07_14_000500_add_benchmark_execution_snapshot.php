...
SELECT jsonb_typeof(value) = 'object'
 AND (SELECT array_agg(key ORDER BY key) FROM jsonb_object_keys(value) key) = ARRAY['adapter_id','currency','dataset_content_hash','dataset_id','dataset_type','dataset_version','manifest_base_prefix','manifest_locator','manifest_sha256','model_versions','normative_version','organization_id','pipeline_version','price_version','prompt_version','schema_version','settings_limits','settings_organization_id','settings_scope','settings_snapshot_hash','settings_snapshot_id','settings_snapshot_version']::text[]
 ...
 AND ((value->>'dataset_type' = 'acceptance' AND value->>'manifest_base_prefix' = ('org-' || (value->>'organization_id') || '/estimate-generation/benchmarks/acceptance/'))
   OR (value->>'dataset_type' IN ('development','regression') AND (value->>'manifest_base_prefix') ~ ('^org-' || (value->>'organization_id') || '/estimate-generation/benchmark-imports/sha256-[a-f0-9]{64}/objects/$')))
 AND ((value->>'dataset_type' = 'acceptance' AND (value->>'manifest_locator') ~ ('^s3://org-' || (value->>'organization_id') || '/estimate-generation/benchmarks/acceptance/[A-Za-z0-9._/-]+[.]json$'))
   OR (value->>'dataset_type' IN ('development','regression') AND (value->>'manifest_locator') ~ ('^s3://org-' || (value->>'organization_id') || '/estimate-generation/benchmark-imports/sha256-[a-f0-9]{64}/manifest/[a-f0-9]{64}[.]json$')))
 ...
 AND value->>'settings_scope' IN ('global','organization')
 AND (value->>'settings_snapshot_hash') ~ '^[a-f0-9]{64}$'
 AND jsonb_typeof(value->'settings_limits') = 'object'
...
DECLARE settings_row estimate_generation_setting_snapshots%ROWTYPE;
BEGIN
  ...
  SELECT * INTO settings_row FROM estimate_generation_setting_snapshots
   WHERE scope = 'organization' AND organization_id = NEW.organization_id ORDER BY version DESC LIMIT 1;
  ...
  IF NOT eg_benchmark_execution_snapshot_valid_v1(NEW.execution_snapshot)
     ...
     OR NEW.execution_snapshot->>'dataset_type' <> dataset_row.dataset_type
     OR settings_row.id IS NULL
     OR (NEW.execution_snapshot->>'settings_snapshot_id')::bigint <> settings_row.id
     OR (NEW.execution_snapshot->>'settings_snapshot_version')::integer <> settings_row.version
     OR NEW.execution_snapshot->>'settings_scope' <> settings_row.scope
     OR (NEW.execution_snapshot->>'settings_organization_id') IS DISTINCT FROM to_jsonb(settings_row.organization_id)#>>'{}'
     OR NEW.execution_snapshot->>'settings_snapshot_hash' <> settings_row.snapshot_hash
     OR NEW.execution_snapshot->'model_versions' <> settings_row.snapshot->'models'
     OR NEW.execution_snapshot->'settings_limits' <> settings_row.snapshot->'limits'
     OR NEW.execution_snapshot->>'currency' <> settings_row.currency THEN
