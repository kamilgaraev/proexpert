...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::getDriverName() === 'pgsql') {
            ...
            DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_snapshot_safe_ck CHECK (NOT (price_snapshot ?| ARRAY['prompt','messages','request','response','error','filename','path','secret','token','content','api_key','authorization']))");
            ...
            DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_optional_rates_ck CHECK ((NOT price_snapshot ? 'reasoning_per_million' OR price_snapshot->>'reasoning_per_million' ~ '^(0|[1-9][0-9]{0,9})(\\.[0-9]{1,8})?$') AND (NOT price_snapshot ? 'image_unit' OR price_snapshot->>'image_unit' ~ '^(0|[1-9][0-9]{0,9})(\\.[0-9]{1,8})?$') AND (NOT price_snapshot ? 'page_unit' OR price_snapshot->>'page_unit' ~ '^(0|[1-9][0-9]{0,9})(\\.[0-9]{1,8})?$'))");
            ...
            DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_counter_tariffs_ck CHECK (pricing_status <> 'available' OR ((page_count = 0 OR price_snapshot ? 'page_unit') AND (image_count = 0 OR price_snapshot ? 'image_unit') AND (reasoning_tokens = 0 OR price_snapshot ? 'reasoning_per_million')))");
            DB::unprepared(<<<'SQL'
