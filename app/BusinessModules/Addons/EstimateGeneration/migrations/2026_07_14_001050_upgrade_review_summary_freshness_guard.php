...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION eg_review_summary_source_guard() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS eg_review_summary_source_guard_trg ON estimate_generation_sessions;
...
SQL);
    ...
    public function down(): void
    {
        ...
        DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION eg_review_summary_source_guard() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE content_version text; source_version text; classifier_version integer;
BEGIN
    IF NEW.draft_payload->'local_estimates' IS DISTINCT FROM OLD.draft_payload->'local_estimates' THEN
        content_version := NEW.draft_payload #>> '{quality_summary,content_version}';
        source_version := NEW.draft_payload #>> '{quality_summary,review_items,source_version}';
        classifier_version := COALESCE((NEW.draft_payload #>> '{quality_summary,review_items,classifier_version}')::integer, 0);
        IF content_version IS NULL OR content_version = COALESCE(OLD.draft_payload #>> '{quality_summary,content_version}', '') OR content_version !~ '^sha256:[0-9a-f]{64}$' OR source_version IS DISTINCT FROM content_version OR classifier_version <> 1 THEN
            RAISE EXCEPTION 'estimate_generation.review_summary_source_version_stale';
        END IF;
    END IF;
    RETURN NEW;
