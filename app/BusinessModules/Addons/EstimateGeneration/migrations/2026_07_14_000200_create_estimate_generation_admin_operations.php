...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::getDriverName() === 'pgsql') {
            ...
            DB::statement("ALTER TABLE estimate_generation_admin_operations ADD CONSTRAINT eg_admin_operations_idempotency_key_ck CHECK (idempotency_key ~ '^[A-Za-z0-9._:-]{16,80}$')");
            DB::unprepared("ALTER TABLE estimate_generation_admin_operations ADD CONSTRAINT eg_admin_operations_result_shape_ck CHECK (result IS NULL OR (octet_length(result::text) <= 512 AND jsonb_typeof(result) = 'object' AND result ?& ARRAY['successful','message_key'] AND (result - ARRAY['successful','message_key']) = '{}'::jsonb AND jsonb_typeof(result->'successful') = 'boolean' AND jsonb_typeof(result->'message_key') = 'string' AND char_length(result->>'message_key') BETWEEN 1 AND 80 AND result->>'message_key' IN ('estimate_generation.failure_resolved','estimate_generation.admin_operation_not_found','estimate_generation.failure_resolution_state_conflict') AND ((result->>'successful')::boolean = (result->>'message_key' = 'estimate_generation.failure_resolved'))))");
        }
    }
