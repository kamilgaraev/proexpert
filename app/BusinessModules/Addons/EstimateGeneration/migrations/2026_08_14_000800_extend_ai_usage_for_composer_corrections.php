...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_operation_ck CHECK (operation IN ('ocr','vision','rerank','completeness_review','project_synthesis','estimate_composition','estimate_audit','estimate_composer_correction')) NOT VALID");
        DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_stage_operation_ck CHECK ((stage = 'understand_documents' AND operation IN ('ocr','vision','project_synthesis')) OR (stage = 'checking_geometry' AND operation = 'vision') OR (stage = 'plan_work_items' AND operation = 'estimate_composition') OR (stage = 'match_normatives' AND operation = 'rerank') OR (stage = 'validate_draft' AND operation IN ('completeness_review','estimate_audit','estimate_composer_correction'))) NOT VALID");
    ...
    public function down(): void
    {
        ...
        DB::statement('ALTER TABLE estimate_generation_ai_usage DROP CONSTRAINT IF EXISTS eg_usage_operation_ck');
        DB::statement('ALTER TABLE estimate_generation_ai_usage DROP CONSTRAINT IF EXISTS eg_usage_stage_operation_ck');
        DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_stage_ck CHECK (stage IN ('understand_documents','checking_geometry','plan_work_items','match_normatives','validate_draft'))");
        DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_operation_ck CHECK (operation IN ('ocr','vision','rerank','completeness_review','project_synthesis','estimate_composition','estimate_audit'))");
        DB::statement("ALTER TABLE estimate_generation_ai_usage ADD CONSTRAINT eg_usage_stage_operation_ck CHECK ((stage = 'understand_documents' AND operation IN ('ocr','vision','project_synthesis')) OR (stage = 'checking_geometry' AND operation = 'vision') OR (stage = 'plan_work_items' AND operation = 'estimate_composition') OR (stage = 'match_normatives' AND operation = 'rerank') OR (stage = 'validate_draft' AND operation IN ('completeness_review','estimate_audit')))");
    }
};
