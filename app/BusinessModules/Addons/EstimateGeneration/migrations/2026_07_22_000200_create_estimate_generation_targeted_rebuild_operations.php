            ...
            ADD CONSTRAINT eg_targeted_rebuild_result_ck CHECK (
                ...
                AND (
                    result_delta = '{}'::jsonb OR (
                        jsonb_path_exists(result_delta, '$.target_package')
                ...
                AND (
                    safe_arbiter_review = '{}'::jsonb OR (
                        jsonb_path_exists(safe_arbiter_review, '$.mode')
                        AND jsonb_path_exists(safe_arbiter_review, '$.status')
                        AND jsonb_path_exists(safe_arbiter_review, '$.outcome')
                        AND jsonb_path_exists(safe_arbiter_review, '$.input_hash')
                        AND jsonb_path_exists(safe_arbiter_review, '$.schema_version')
        ...
        DB::statement(<<<'SQL'
            ...
            ADD CONSTRAINT eg_targeted_rebuild_lifecycle_ck CHECK (
                attempt_count BETWEEN 0 AND 1000 AND (
                    ...
                    OR (status = 'reviewed' AND attempt_count >= 1 AND lease_token IS NULL AND lease_expires_at IS NULL AND result_delta <> '{}'::jsonb AND safe_arbiter_review->>'status' = 'reviewed' AND safe_arbiter_review->>'outcome' IN ('passed','confirmed_scope_only') AND reviewed_at IS NOT NULL AND finished_at IS NULL)
                    OR (status = 'committed' AND attempt_count >= 1 AND lease_token IS NULL AND lease_expires_at IS NULL AND result_delta <> '{}'::jsonb AND safe_arbiter_review->>'status' = 'reviewed' AND safe_arbiter_review->>'outcome' IN ('passed','confirmed_scope_only') AND reviewed_at IS NOT NULL AND finished_at IS NOT NULL)
                    OR (status = 'human_review' AND attempt_count >= 1 AND lease_token IS NULL AND lease_expires_at IS NULL AND safe_arbiter_review->>'status' IN ('reviewed','unavailable') AND safe_arbiter_review->>'outcome' = 'human_review' AND finished_at IS NOT NULL AND ((result_delta = '{}'::jsonb AND reviewed_at IS NULL) OR (result_delta <> '{}'::jsonb AND reviewed_at IS NOT NULL)))
