...
final readonly class EloquentDocumentProcessingUnitStore implements DocumentProcessingUnitStore
{
    ...
    {
        $model = EstimateGenerationProcessingUnit::query()->firstOrCreate(
    ...
    {
        $model = EstimateGenerationProcessingUnit::query()->find($unitId);
    ...
    {
        $unit = EstimateGenerationProcessingUnit::query()->with('document')->find($claim->unitId);
    ...
    public function claim(int $unitId, string $sourceVersion, DateTimeImmutable $now, DateTimeImmutable $leaseExpiresAt, int $maxAttempts): DocumentProcessingUnitClaim
    {
        return $this->database->transaction(function () use ($unitId, $sourceVersion, $now, $leaseExpiresAt, $maxAttempts): DocumentProcessingUnitClaim {
            $unit = EstimateGenerationProcessingUnit::query()->with('document')->lockForUpdate()->find($unitId);
            ...
            if ($unit->status === DocumentProcessingUnitStatus::Completed
                || ($unit->status === DocumentProcessingUnitStatus::Running && $unit->lease_expires_at?->toDateTimeImmutable() > $now)
                || (int) $unit->attempt_count >= $maxAttempts
                || $leaseExpiresAt <= $now) {
                return new DocumentProcessingUnitClaim($unitId, false);
            }
            ...
            return new DocumentProcessingUnitClaim($unitId, true, $token);
        }, 3);
    }
    ...
    public function complete(DocumentProcessingUnitClaim $claim, string $outputVersion, int $outputCount, DateTimeImmutable $now): bool
    {
        return EstimateGenerationProcessingUnit::query()
            ->whereKey($claim->unitId)->where('status', DocumentProcessingUnitStatus::Running->value)
    ...
    public function publish(DocumentProcessingUnitClaim $claim, DocumentUnitOutput $output, DateTimeImmutable $now): bool
    {
        return $this->database->transaction(function () use ($claim, $output, $now): bool {
            $unit = EstimateGenerationProcessingUnit::query()->with('document')->lockForUpdate()->find($claim->unitId);
            ...
            EstimateGenerationDocumentPage::query()->updateOrCreate(
                ['document_id' => $unit->document_id, 'page_number' => $unit->unit_index],
                ['processing_unit_id' => $unit->id, 'organization_id' => $unit->organization_id, 'project_id' => $unit->project_id, 'session_id' => $unit->session_id, 'width' => $output->width, 'height' => $output->height, 'rotation' => $output->rotation, 'text' => $output->text, 'text_hash' => $output->text !== '' ? hash('sha256', $output->text) : null, 'confidence' => $output->confidence, 'normalized_payload' => $output->normalizedPayload, 'quality_flags' => []],
            );
    ...
    {
        return $leaseExpiresAt > $now && EstimateGenerationProcessingUnit::query()
            ->whereKey($claim->unitId)->where('status', DocumentProcessingUnitStatus::Running->value)
            ->where('claim_token', $claim->token)->where('lease_expires_at', '>', $now)
    ...
    {
        EstimateGenerationProcessingUnit::query()->where('document_id', $documentId)->where('source_version', '<>', $currentSourceVersion)
