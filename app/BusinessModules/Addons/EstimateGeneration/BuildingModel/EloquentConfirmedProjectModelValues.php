...
final readonly class EloquentConfirmedProjectModelValues
{
    ...
    public function forModel(stdClass $model): array
    {
        ...
        $bindings = [];
        foreach ($this->database->table('estimate_generation_project_model_evidence_bindings as b')->join('estimate_generation_evidence as ev', 'ev.id', '=', 'b.evidence_id')->join('estimate_generation_project_model_entities as e', 'e.id', '=', 'b.entity_id')->leftJoin('estimate_generation_project_model_assertions as a', 'a.id', '=', 'b.assertion_id')->leftJoin('estimate_generation_project_model_corrections as c', 'c.id', '=', 'b.correction_id')->leftJoin('estimate_generation_project_model_assertions as ca', 'ca.id', '=', 'c.assertion_id')->where('b.building_model_id', $model->id)->where('b.organization_id', $scope[0])->where('b.project_id', $scope[1])->where('b.session_id', $scope[2])->where('b.source_version', $scope[3])->whereNull('ev.invalidated_at')->orderBy('b.id')->get(['b.*', 'e.stable_key as entity_stable_key', 'a.stable_key as assertion_stable_key', 'ca.stable_key as correction_assertion_stable_key', 'c.stable_key as correction_stable_key', 'ev.source_version as evidence_source_version_actual', 'ev.invalidation_version as evidence_invalidation_version_actual', 'ev.type as evidence_type_actual', 'ev.source_type as evidence_source_type_actual', 'ev.source_ref as evidence_source_ref_actual', 'ev.producer_name as evidence_producer_name_actual', 'ev.producer_version as evidence_producer_version_actual', 'ev.locator as evidence_locator_actual', 'ev.value as evidence_value_actual', 'a.payload as assertion_payload', 'ca.payload as correction_assertion_payload']) as $row) {
