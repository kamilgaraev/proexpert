...
class EstimateNormativeMatcher
{
    ...
    public function matchWorkItem(array $workItem, array $context = [], int $limit = 5): ?array
    {
        ...
        $ranked = $candidates
            ->map(fn (EstimateNorm $norm): array => $this->scoreNorm($norm, $workItem, $context, $tokens, $version->id))
     ...
     */
    private function scoreNorm(EstimateNorm $norm, array $workItem, array $context, array $tokens, int $versionId): array
    {
        ...
        if (($workItem['unit'] ?? null) !== null && $this->sameUnit((string) $workItem['unit'], (string) $norm->unit)) {
            $score += 8;
            $reasons[] = 'unit';
        }
        ...
        $resources = $this->resourcesForNorm($norm, $versionId);
        ...
        return [
            ...
            'warnings' => $this->warningsForCandidate($confidence, $resourceCount, $pricedCount),
            'resources' => $resources,
        ];
    }
     ...
     */
    private function resourcesForNorm(EstimateNorm $norm, int $versionId): array
    {
        ...
        $prices = EstimateResourcePrice::query()
        ...
        foreach ($resources as $resource) {
            ...
            $price = $prices->get($resource->resource_code);
            $payload = [
     ...
     * @return array<int, string>
     */
    private function warningsForCandidate(float $confidence, int $resourceCount, int $pricedCount): array
    {
        $warnings = [];
