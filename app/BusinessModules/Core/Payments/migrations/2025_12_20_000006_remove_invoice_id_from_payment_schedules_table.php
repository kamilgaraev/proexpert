    ...
    public function up(): void
    {
        ...
        if ($foreignKey && isset($foreignKey->constraint_name)) {
            try {
                ...
                DB::statement("ALTER TABLE payment_schedules DROP CONSTRAINT IF EXISTS \"{$constraintName}\" CASCADE");
            } catch (\Exception $e) {
                ...
                foreach ($standardNames as $name) {
                    try {
                        DB::statement("ALTER TABLE payment_schedules DROP CONSTRAINT IF EXISTS \"{$name}\" CASCADE");
                        break;
                    } catch (\Exception $e2) {
                        // Продолжаем попытки
                    }
