...
class MdmQualityService
{
    ...
    public function evaluate(string $entityType, array $attributes): array
    {
        $issues = [];
        $requiredFields = $this->requiredFields($entityType);
        ...
        foreach ($requiredFields as $field) {
            if (($attributes[$field] ?? null) === null || trim((string) $attributes[$field]) === '') {
                $issues[] = $this->issue($field . '_required', $field, 25);
            }
        }
