...
final readonly class ReportingArtifactTransfer
{
    ...
    )
    {
        ...
        if (($ids[$kind] ?? null) !== $artifactId || $status !== 'artifact_transferred' || $schemaVersion !== '1.0.0' || preg_match('/^[a-f0-9]{40}$/', $releaseSha) !== 1 || preg_match('/^[a-f0-9]{40}$/', $sourceCommitSha) !== 1 || preg_match('/^[a-f0-9]{40}$/', $activationCommitSha) !== 1 || ($adminEvidenceCommitSha !== null && preg_match('/^[a-f0-9]{40}$/', $adminEvidenceCommitSha) !== 1) || (($kind === 'release') !== ($adminEvidenceCommitSha !== null)) || preg_match('/^[a-f0-9]{64}$/', $sourceSha256) !== 1 || preg_match('/^[a-f0-9]{64}$/', $destinationSha256) !== 1 || preg_match('/^[a-f0-9]{64}$/', $schemaSha256) !== 1 || preg_match('/^[a-f0-9]{64}$/', $transferSchemaSha256) !== 1) {
