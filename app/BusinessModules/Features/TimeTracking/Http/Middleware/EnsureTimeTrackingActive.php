...
namespace App\BusinessModules\Features\TimeTracking\Http\Middleware;
...
final class EnsureTimeTrackingActive
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($organizationId <= 0) {
            return $this->errorResponse(
                $request,
                trans_message('time_tracking.mobile.errors.organization_missing'),
                403
            );
        }
        ...
        foreach (['time-tracking', 'project-management'] as $moduleSlug) {
            if (!$accessController->hasModuleAccess($organizationId, $moduleSlug)) {
                return $this->errorResponse(
                    $request,
                    trans_message('time_tracking.mobile.errors.module_not_active'),
                    403,
