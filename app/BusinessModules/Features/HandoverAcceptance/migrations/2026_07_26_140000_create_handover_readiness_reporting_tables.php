    ...
    public function up(): void
    {
        ...
        Schema::create('handover_evidence_events', function (Blueprint $table): void {
            ...
            $table->unique(
                ...
                'handover_evidence_source_event_unique',
            );
            $table->index(
        ...
        foreach ([
            ...
            "ALTER TABLE handover_gate_versions ADD CONSTRAINT handover_gate_empty_requirement_check CHECK (NOT explicitly_empty_requirements OR (jsonb_array_length(required_checklist_codes) = 0 AND jsonb_array_length(required_document_codes) = 0))",
            "ALTER TABLE handover_evidence_events ADD CONSTRAINT handover_evidence_hash_check CHECK (evidence_hash ~ '^[a-f0-9]{64}$')",
            "ALTER TABLE handover_evidence_events ADD CONSTRAINT handover_evidence_causation_check CHECK ((event_type IN ('finding_resolved','inspection_resulted')) = (causation_event_id IS NOT NULL))",
