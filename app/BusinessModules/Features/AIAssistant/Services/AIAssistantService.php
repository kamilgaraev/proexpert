    ...
    protected function formatContextSection(string $key, $value): string
    {
        ...
        if ($key === 'generate_report' && isset($value['report_type'])) {
            ...
            if (isset($value['pdf_url'])) {
                $output .= "🔴 PDF ССЫЛКА (ОБЯЗАТЕЛЬНО ПОКАЖИ ПОЛЬЗОВАТЕЛЮ):\n";
                $output .= $value['pdf_url'] . "\n";
                $output .= "(Ссылка действительна 24 часа)\n\n";
                $output .= "⚠️ ВАЖНО: Эту ссылку НУЖНО показать пользователю в ответе!\n\n";
            }
        }
