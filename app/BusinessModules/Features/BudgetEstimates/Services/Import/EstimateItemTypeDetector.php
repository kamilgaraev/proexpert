...
class EstimateItemTypeDetector
{
    ...
    private function detectTypeByCode(string $code): ?string
    {
        ...
        // Формат: Х-ХХХ-ХХ или Х-ХХХ-ХХХ (средний разряд работы/машинистов)
        if (preg_match('/^\d-\d{3}-\d{2,3}$/u', $code)) {
            return 'labor';
        }
        
        // ⭐ МАТЕРИАЛЫ: СЦМ (Сборники цен на материалы)
        // Формат: СЦМ-XXX-XXXX или СЦМ-XXXX-XXXX
        if (preg_match('/^СЦМ-\d{3,4}-\d{3,4}$/ui', $code)) {
            return 'material';
        }
        
        // ⭐ РАБОТЫ: ГЭСН, ФЕР, ТЕР (формат ГЭСН01-01-012-20 или 01-01-012-20)
        // ВСЕГДА возвращаем 'work' для кодов с префиксами ГЭСН/ФЕР/ТЕР
        if (preg_match('/^(ГЭСН|ГСН|ФЕР|ТЕР)/ui', $code)) {
            return 'work';
        }
        
        // РАБОТЫ: без префикса, но в формате XX-XX-XXX-XX
