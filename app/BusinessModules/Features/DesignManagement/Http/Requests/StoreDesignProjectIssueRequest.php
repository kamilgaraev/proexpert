...
final class StoreDesignProjectIssueRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'annotations' => ['nullable', 'array:schema_version,width,height,strokes'],
            'annotations.schema_version' => ['required_with:annotations', 'integer', Rule::in([1])],
            'annotations.width' => ['required_with:annotations', 'numeric', 'gt:0', 'max:16384'],
            'annotations.height' => ['required_with:annotations', 'numeric', 'gt:0', 'max:16384'],
            'annotations.strokes' => ['required_with:annotations', 'array', 'max:100'],
            'annotations.strokes.*' => ['array:kind,color,width,points'],
            'annotations.strokes.*.kind' => ['required', 'string', Rule::in(['line', 'arrow', 'freehand'])],
            'annotations.strokes.*.color' => ['required', 'string', 'regex:/^#[0-9a-fA-F]{6}$/'],
            'annotations.strokes.*.width' => ['required', 'numeric', 'between:1,20'],
            'annotations.strokes.*.points' => ['required', 'array', 'min:2', 'max:1000'],
            'annotations.strokes.*.points.*' => ['array:x,y'],
            'annotations.strokes.*.points.*.x' => ['required', 'numeric', 'between:0,1'],
            'annotations.strokes.*.points.*.y' => ['required', 'numeric', 'between:0,1'],
