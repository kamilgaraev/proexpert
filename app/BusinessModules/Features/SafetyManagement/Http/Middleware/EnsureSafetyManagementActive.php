...
namespace App\BusinessModules\Features\SafetyManagement\Http\Middleware;
...
final class EnsureSafetyManagementActive
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($organizationId <= 0) {
            return $this->errorResponse($request, trans_message('safety_management.errors.organization_missing'), 403);
        }
        ...
        foreach (['safety-management', 'project-management', 'file-management'] as $moduleSlug) {
            if (!$accessController->hasModuleAccess($organizationId, $moduleSlug)) {
                return $this->errorResponse($request, trans_message('safety_management.errors.module_not_active'), 403);
            }
        }
