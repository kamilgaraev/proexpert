...
abstract readonly class ProjectFinanceManagementPnlComponentSource implements ManagementPnlComponentSource
{
    public function snapshots(ReportScope $scope, ReportQuery $query): iterable
    {
        ...
        foreach ($rows as $currency => $currencyRows) {
            ...
            foreach ($currencyRows as $row) {
                if ($code === 'budget_plan_fact' && $this->isLaborRow((array) $row->source_refs)) {
                    $excludedLaborRows++;
    ...
    private function isLaborRow(array $sourceRefs): bool
    {
        foreach ($sourceRefs as $sourceRef) {
            if (is_array($sourceRef) && in_array(
                $sourceRef['type'] ?? null,
                ['approved_time_entry', 'project_labor_cost', 'payroll_source_row'],
                true,
            )) {
                return true;
            }
