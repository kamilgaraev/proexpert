...
class PerformanceActReportController extends Controller
{
    ...
    public function exportPdf(int $actId): JsonResponse
    {
        try {
            ...
            Log::error('PerformanceActReport.exportPdf failed', [
                'act_id' => $actId,
                'error' => $e->getMessage()
            ]);
    ...
    public function exportExcel(int $actId): JsonResponse
    {
        try {
            ...
            Log::error('PerformanceActReport.exportExcel failed', [
                'act_id' => $actId,
                'error' => $e->getMessage()
            ]);
    ...
    public function bulkExportExcel(Request $request): JsonResponse
    {
        try {
            ...
            // Для наглядности используем существующий ExcelExporterService или создаем новый через OfficialFormsExportService
            // В данном случае, так как нам нужна объединенная таблица, используем ExcelExporterService
            $headers = [
            ...
            Log::error('PerformanceActReport.bulkExportExcel failed', [
                'error' => $e->getMessage()
            ]);
