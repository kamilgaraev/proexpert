# Правила маппинга документов МОСТ и 1С
...
Документ фиксирует правила обмена документами МОСТ и 1С. Он опирается на разделение трех осей: операционный статус МОСТ, sync status exchange-слоя и учетный статус 1С.
...
- 1С не должна обновлять обратно операционные статусы МОСТ, если field ownership принадлежит МОСТ.

## Глобальная модель полей

| Группа полей | Владелец | Правило обратного обновления |
| --- | --- | --- |
| Операционный lifecycle | МОСТ | 1С не меняет |
| Бухгалтерская приемка и проведение | 1С | МОСТ хранит отдельно как accounting status |
| Номера и даты МОСТ | МОСТ | 1С может вернуть внешний номер, но не перезаписывает локальный |
| Внешний id, учетный код, accounting reference | 1С | сохраняется в mapping/status |
| Сумма, строки, контрагент, проект после отправки | владелец документа МОСТ до отправки, после отправки через версию/корректировку | не обновлять тихо из 1С |
| Банковский факт | банк или 1С как внешний факт | не равен локальному `paid` без сверки |
| Payroll legal result | 1С/ЗУП | МОСТ хранит только acceptance/rejection и source rows |

## Договоры

| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по операционному договору; 1С по учетной карточке и бухгалтерскому отражению |
...
| Field-level ownership | статус договора, проект, предмет, workflow - МОСТ; external id, accounting status - 1С |
| Табличные части | распределение по проектам/сметам, график платежей, статьи затрат при необходимости |
| Статусы МОСТ | `draft`, `active`, `completed`, `on_hold`, `terminated` |
...
| Не бухгалтерский факт | `active` или `completed` в МОСТ не означает проведение договора в 1С |

## Акты

| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по выполнению работ и операционной приемке; 1С по учетному проведению |
...
| Field-level ownership | состав работ, период, сумма и approval - МОСТ; accounting posted status - 1С |
| Табличные части | строки работ/сметных позиций, суммы, НДС, аналитика проекта/статьи |
| Статусы МОСТ | `draft`, `approved`, `signed` для текущего `ContractPerformanceAct`; если есть расширенный актовый workflow, статусная ось остается операционной |
...
| Не бухгалтерский факт | `approved` и `signed` в МОСТ не равны бухгалтерскому `posted` |

## Платежные заявки

| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по заявке, согласованию и планированию платежа |
...
| Field-level ownership | approval workflow, due date, приоритет, связь с проектом - МОСТ |
| Табличные части | распределение по статьям бюджета, объектам и основаниям |
| Статусы МОСТ | `draft`, `submitted`, `pending_approval`, `approved`, `scheduled`, `rejected`, `cancelled` |
...
| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по операционному платежному документу; банк по факту движения денег; 1С по учетному отражению |
...
| Field-level ownership | workflow и связь с заявкой - МОСТ; accounting status - 1С; bank execution - банк |
| Табличные части | распределение суммы по основаниям, проектам, статьям |
| Статусы МОСТ | `approved`, `scheduled`, `paid`, `partially_paid`, `cancelled` и другие из PaymentDocument |
...
| Не бухгалтерский факт | `paid` в МОСТ не равно бухгалтерскому проведению без банковского и/или 1С подтверждения |

## Банковские факты

| Правило | Значение |
| --- | --- |
| Owner/source of truth | Банк по факту списания/поступления; 1С по бухгалтерскому отражению; МОСТ по операционной сверке |
...
| Field-level ownership | банковский id и факт движения - внешний источник; match status - МОСТ |
| Табличные части | сопоставление с несколькими платежными документами при частичной оплате |
| Статусы МОСТ | transaction `pending`, `processing`, `completed`, `failed`, `cancelled`, `refunded` |
...
| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по цепочке request -> supplier request/proposal -> purchase order; 1С по учетному поступлению/счету |
...
| Field-level ownership | выбор поставщика, статусы снабжения, связи с заявками - МОСТ |
| Табличные части | позиции заказа, количество, цена, НДС, материалы, склад, проект |
| Статусы МОСТ | order `draft`, `sent`, `confirmed`, `in_delivery`, `partially_delivered`, `delivered`, `cancelled` |
| Sync status | `queued`, `sent`, `accepted`, `posted`, `rejected`, `failed`, `requires_mapping`, `conflict` |
...
| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по оперативному складу стройки; 1С по официальному складскому и стоимостному учету, если включен |
...
| Field-level ownership | оперативные остатки, резервы, mobile scan - МОСТ; учетная партия/стоимость - 1С |
| Табличные части | строки материалов, партии/серии при наличии, количество, цена, склад, проект |
| Статусы МОСТ | movement immutable; delivery `requested`, `processing`, `reserved`, `in_transit`, `delivered`, `accepted`, `problem`, `cancelled` |
| Sync status | `queued`, `sent`, `accepted`, `posted`, `rejected`, `failed`, `conflict` |
...
| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по факту инвентаризации площадки; 1С по учетной корректировке |
...
| Field-level ownership | фактический пересчет и комиссии - МОСТ; учетная корректировка - 1С |
| Табличные части | material, expected qty, actual qty, diff, unit, batch/serial при наличии |
| Статусы МОСТ | `draft`, `in_progress`, `completed`, `approved`, `cancelled` |
| Sync status | `queued`, `sent`, `accepted`, `posted`, `rejected`, `failed`, `conflict` |
...
| Не бухгалтерский факт | `approved` в МОСТ не означает учетную корректировку 1С |

## Бюджетные корректировки

| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по управленческому бюджету и сметам; 1С по учетной аналитике затрат |
...
| Field-level ownership | бюджетный workflow, версии сметы, управленческие лимиты - МОСТ |
| Табличные части | статьи бюджета, ЦФО, проектные разрезы, суммы |
| Статусы МОСТ | зависят от бюджетного workflow, но остаются операционными |
...
| Не бухгалтерский факт | утвержденный бюджет МОСТ не является бухгалтерской проводкой |

## Payroll export packages

| Правило | Значение |
| --- | --- |
| Owner/source of truth | МОСТ по source rows явки/выработки; 1С/ЗУП по юридической зарплате, налогам и взносам |
...
| Field-level ownership | явка, выработка, source rows - МОСТ; начисление зарплаты, НДФЛ, взносы, отчетность - 1С/ЗУП |
| Табличные части | строки payroll source по сотрудникам, датам, проектам, нарядам |
| Статусы МОСТ | `draft`, `validated`, `locked` для периода и статусы export package |
| Sync status | `queued`, `sent`, `accepted`, `rejected`, `failed`, `requires_mapping`, `conflict` |
