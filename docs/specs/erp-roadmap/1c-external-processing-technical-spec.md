# ТЗ для внешней обработки 1С МОСТ
...
Документ описывает требования к внешней обработке 1С, расширению 1С или иному 1С-коннектору, который будет обмениваться данными с МОСТ.

Документ можно передать 1С-разработчику как техническое задание и промпт для реализации. Подрядчик должен реализовать сторону 1С так, чтобы она работала с МОСТ без устных уточнений по базовым сценариям: настройка подключения, проверка совместимости, прием сообщений, возврат статусов, защита от дублей, журналирование, безопасные ошибки, mapping и ручная сверка.
...
Цель интеграции - передавать между МОСТ и 1С операционные данные строительной ERP, учетные подтверждения, справочники, статусы обмена и данные для сверки без превращения МОСТ в параллельную бухгалтерскую систему.

Внешняя обработка 1С должна:

- принимать от МОСТ утвержденные документы, справочники и пакеты источников;
...
- МОСТ = construction ERP / operational source of truth.
...
- `approved` в МОСТ не означает, что документ принят или проведен в 1С;
- `signed` в МОСТ не означает юридически значимую подпись без ЭДО/КЭП;
- `paid` в МОСТ не означает бухгалтерское проведение без банка и/или 1С;
...
Внешняя обработка 1С является адаптером между 1С и МОСТ. Она не должна менять бизнес-границы и не должна скрыто создавать учетные факты без явного документа, статуса и журнала.
...
| `protocol_version` | да | Версия протокола обмена с МОСТ. Первый целевой вариант: `1.0` |
| `environment` | да | `production`, `test` или `sandbox` |
| `profile_code` | да | Код integration profile из МОСТ |
| `base_code` | да | Код базы 1С в МОСТ |
...
| `token_or_key` | да, если auth не `none` | Секрет для авторизации запросов МОСТ |
...
| `callback_url` | нет | URL МОСТ для асинхронных ответов 1С, если сценарий callback включен |
...
МОСТ поддерживает несколько организаций, юридических лиц, баз 1С и integration profiles.

Обработка должна принимать и сохранять контекст:

| Поле | Назначение |
| --- | --- |
| `organization_id` | Tenant/организация МОСТ |
| `legal_entity_id` | Юрлицо, если МОСТ передает его для бухгалтерского документа |
...
| `allowed_scopes` | Scopes, разрешенные в профиле МОСТ |
...
- idempotency key должен учитывать environment или приходить из МОСТ уже уникальным для environment;
- один и тот же `external_id` в разных базах 1С не является конфликтом;
- один `external_id` в одной базе, одном scope и одном типе объекта, связанный с несколькими МОСТ-объектами, является duplicate mapping conflict;
- отключение или пауза профиля не удаляет журнал и mappings.

### 3.4. Авторизация

Для запросов МОСТ -> 1С:

- основной вариант - HTTPS + `Authorization: Bearer <token>`;
- альтернативный вариант - HTTPS + Basic Auth, если согласовано в profile;
- секрет выбирается на стороне МОСТ из `OneCProfileSecret`;
...
Для запросов 1С -> МОСТ:

- использовать выданный МОСТ integration token или иной согласованный секрет;
...
МОСТ уже имеет endpoint проверки профиля:
...
| `entity_type`, `entity_id` | Сущность МОСТ |
...
Endpoint должен принимать JSON и возвращать JSON. Все ответы должны быть безопасными для сохранения в журнале МОСТ после санитизации.
...
| `operation_id` | да | Идентификатор операции МОСТ |
...
| `entity_type`, `entity_id` | да | Идентификация объекта МОСТ |
...
- если payload изменился после `sent`, МОСТ должен прислать новую версию и новый `idempotency_key`;
...
Если 1С должна отправить событие в МОСТ асинхронно, она использует тот же envelope. До появления отдельного callback endpoint в конкретной реализации 1С должна уметь вернуть финальный статус синхронным ответом на delivery request.

Событие 1С -> МОСТ используется для:
...
- 1С не должна менять операционный workflow МОСТ напрямую;
- 1С возвращает accounting status, external id, accounting code и safe error summary;
- МОСТ решает, какие доменные поля можно обновить;
...
Если конкретная база 1С не поддерживает часть scopes, она должна честно вернуть фактический список в metadata. МОСТ поставит профиль в `missing_scope`, если профиль требует scope, которого нет в `supported_scopes`.

### 6.2. Контрагенты

| Параметр | Правило |
| --- | --- |
| Source of truth | МОСТ по операционной карточке и связям; 1С по учетным реквизитам |
...
| Нельзя | Тихо перезаписывать МОСТ-owned контакты и workflow из 1С |
...
- МОСТ владеет lifecycle, проектной привязкой, предметом и согласованием;
...
- МОСТ владеет фактом выполнения работ и операционной приемкой;
- 1С владеет бухгалтерским отражением;
- `approved` или `signed` в МОСТ не равны `posted` в 1С;
...
- МОСТ владеет заявкой, approval workflow, календарем и приоритетом;
- банк владеет фактом исполнения;
- 1С владеет учетным отражением платежа;
- `paid` в МОСТ не равно accounting `posted` без подтверждения банка и/или 1С.
...
МОСТ владеет оперативным складом стройки:
...
Операционный остаток МОСТ не является бухгалтерским остатком 1С.

### 6.7. Payroll-source

МОСТ владеет payroll-source:
...
- не рассчитывать юридическую зарплату в МОСТ и не возвращать в МОСТ лишние персональные/налоговые детали.
...
- МОСТ блокирует сообщение до ручного сопоставления;
...
- сохранить событие в журнале 1С и МОСТ;
- после ручного решения выполнить requeue с audit reason.

### 7.4. Reconciliation

Reconciliation нужен для сверки МОСТ vs 1С vs банк/ЭДО/ЗУП.

Сверка должна выявлять:

- документ есть в МОСТ, но не принят 1С;
- документ принят 1С, но не имеет mapping в МОСТ;
- суммы, даты или контрагент отличаются;
- платеж исполнен банком, но не отражен в 1С;
- акт проведен в 1С, но операционный документ МОСТ изменился после отправки;
...
Перед повторной отправкой 1С-обработка или МОСТ должны проверить:
...
- МОСТ отправляет payment document с idempotency key;
...
- МОСТ получает retryable safe error `timeout`;
...
- МОСТ отправляет один и тот же документ дважды с одинаковым idempotency key и payload_hash;
...
- МОСТ отправляет повтор с тем же idempotency key, но другим payload_hash или контрольными полями;
...
- МОСТ не делает автоматический retry;
- пользователь видит safe next action;
- после исправления бизнес-данных МОСТ создает новую версию сообщения, если payload изменился.

### 10.11. Payroll-source boundary

- МОСТ отправляет закрытый payroll-source package;
- 1С возвращает `accepted` или `rejected`;
- МОСТ не получает и не рассчитывает юридическую зарплату, НДФЛ, взносы и регламентированную отчетность;
...
12. Какие поля 1С могут обновлять МОСТ без ручной заявки, а какие всегда идут через reconciliation/review.
...
- delivery endpoint принимает envelope МОСТ;
...
- бухгалтерские проводки, налоговый учет и юридический payroll не дублируются в МОСТ;
