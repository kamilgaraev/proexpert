# Очередь обмена, idempotency и retry МОСТ и 1С
...
Документ задает целевую модель очереди обмена МОСТ и 1С. Текущий `OneCBasicExchange` выполняет ручные import/export операции и не содержит production-очередь, ordering, dead-letter, lease-locking и промышленную idempotency-модель.

Границы:

- МОСТ остается операционной ERP.
...
| Message | Конкретное сообщение для 1С или МОСТ | "экспорт акта 123 в базу A" | защищается `idempotency_key` |
...
- если 1С не поддерживает ключ, МОСТ сверяет контрольные поля: external id, номер, дата, сумма, контрагент, организация, hash табличной части;
...
| Payroll legal rejection | нет | после решения HR/payroll owner | МОСТ не считает зарплату сам |
