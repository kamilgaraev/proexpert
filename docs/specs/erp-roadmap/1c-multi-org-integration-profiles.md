...
Эта спецификация описывает целевую модель настроек обмена между ProHelper и 1С для сценариев, где в одной организации ProHelper может быть несколько юридических лиц, несколько баз 1С, разные контуры обмена и разные правила маршрутизации документов.
...
- ProHelper остается операционной строительной ERP и operational source of truth.
- 1С остается бухгалтерским и налоговым source of truth.
- ProHelper не становится accounting-core, не рассчитывает регламентированную бухгалтерию и не подменяет 1С.
- 1С не управляет операционными статусами строительных процессов в ProHelper.

Операционные статусы ProHelper не являются бухгалтерским проведением 1С.

Примеры:

- заявка на оплату в ProHelper может быть согласована, но это не означает, что платежный документ проведен в 1С;
- акт может быть принят в операционном контуре ProHelper, но бухгалтерское отражение остается отдельным статусом 1С;
- складская операция в ProHelper может быть подтверждена на объекте, но налоговый или бухгалтерский учет движения остается в 1С;
- статус `sent` или `accepted` в обмене не равен `posted` в 1С.

Для всех интеграционных сущностей должны сохраняться отдельные оси:

- operational status в ProHelper;
...
`organization` - текущая организация ProHelper, tenant и операционный контур, который уже используется в backend через `organization_id`.

Организация ProHelper может содержать несколько юридических лиц. Поэтому `organization_id` нельзя использовать как единственный идентификатор стороны бухгалтерского обмена.

### 5.2. legal entity

`legal entity` - юридическое лицо внутри организации ProHelper.
...
Юрлицо не равно ProHelper organization. В простом сценарии у организации одно юрлицо, но модель должна поддерживать несколько.

### 5.3. one_c_base

`one_c_base` - конкретная база 1С или логический контур 1С, с которым ProHelper обменивается данными.
...
Одна база 1С может обслуживать несколько юридических лиц одной ProHelper organization, если это явно настроено через разные integration profiles.
...
Юридические лица внутри ProHelper organization.
...
| `organization_id` | fk | да | ProHelper organization |
...
| `outbound_only` | Только ProHelper -> 1С |
| `inbound_only` | Только 1С -> ProHelper |
...
1. Нужен ли отдельный platform-level справочник баз 1С для сценария, где одна физическая база обслуживает несколько ProHelper organizations, или в первой реализации база всегда принадлежит одной organization.
...
- отдельно зафиксировано, что операционные статусы ProHelper не являются бухгалтерским проведением 1С;
