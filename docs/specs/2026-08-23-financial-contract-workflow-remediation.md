...
| FIN-024 | Medium | Needs production evidence | Фактические production-объёмы/SLO и планы тяжёлых запросов статически не доказаны. | Пагинация/chunked backfill и fail-fast migration preflight реализованы; метрики проверяются read-only до/после deploy. | Частично подтверждено | Production migration preflight и все deploy health gates GREEN; публичные `/ready` и `/up` вернули HTTP 200, `ready=true`. SSH остался недоступен (`Connection timed out during banner exchange`), поэтому фактические объёмы, query plans и наблюдаемый SLO остаются residual evidence gap. |
...
- 2026-08-24: первый backend deploy `32681863278` безопасно остановлен до первой миграции на legacy `NULL` в связи договора со сметой; точный red/green regression закрыт PR `proexpert#468`, повторный deploy успешен.
- 2026-08-24: backend, admin и кабинет заказчика последовательно доставлены штатными workflow; post-deploy HTTP smoke пройден без записи данных.

## Поставка

- Backend: commit `48caaecd50125bea4efea42c1027a9a972a7efb0`, PR `proexpert#467`, merge `965f62f2fa55b8cfdfe7cb0cdd25b729b153eee5`; migration hotfix commit `466b11b2096050ecfb003ab17ac13a32b3a74fbe`, PR `proexpert#468`, merge `87495895dfe7622bc33d34b7b39ebb04c329039f`.
- Admin: commit `3ddc567216867ae3039d462fb2c87bfde7cbce4d`, PR `prohelper_admin#92`, merge `99fbb619296788d1c548f0b4cb5a5acf95cc35e6`.
- Кабинет заказчика: commit `3104ad319c231bef11e26b68c1696c83e2c2f1d7`, PR `prohelper_customers#2`, merge `8e35e281d2035ac369c5b2f249784c2685ae2e00`.
- Backend deploy: `32682648525` GREEN; admin deploy: `32682954966` GREEN; кабинет заказчика deploy: `32683075863` GREEN.
- Post-deploy read-only smoke: `https://api.1мост.рф/ready` → HTTP 200 (`ready=true`, `phase=phase_b`), `/up` → HTTP 200; основной сайт, admin и customer roots → HTTP 200; admin `release.json` содержит точный merge SHA `99fbb619296788d1c548f0b4cb5a5acf95cc35e6`.
- Pull-request checks в репозиториях не настроены; все штатные post-merge build/test/deploy workflow завершились успешно.
