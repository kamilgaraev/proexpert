...
Filament использует отдельный закрытый набор прав и не подменяет проектные права пользователя:

| Право | Поверхность |
| --- | --- |
| `estimate_generation.monitor` | Дашборд, сессии, использование, ошибки и контрольные точки |
| `estimate_generation.operate` | Подтверждённые повтор, отмена, архивирование и закрытие ошибки |
| `estimate_generation.datasets` | Наборы данных и их проверка |
| `estimate_generation.benchmarks` | Запуски benchmark и просмотр безопасного отчёта |
| `estimate_generation.settings` | Версионированные настройки моделей, лимитов и правил |
| `estimate_generation.budgets` | Дневные и месячные лимиты стоимости |

Матрица ролей следует принципу минимальных прав: `support_operator` получает monitor+operate, `qa_engineer` — monitor+datasets+benchmarks, `security_auditor` — только monitor, `super_admin` — все шесть прав. Настройки доступны только при одновременном наличии `settings` и `budgets`. Изменяющие действия требуют отдельного права, подтверждения, актуальной версии состояния и записи аудита.
