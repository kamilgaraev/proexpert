...
### Tenant-scoped availability перед запуском

Опубликованный отчёт присутствует в едином каталоге для каждого tenant, которому он доступен по модулю и правам. Отсутствие фактов у конкретной организации или проекта не является основанием глобально скрывать определение отчёта.

Перед созданием run dedicated options endpoint проверяет выбранный период и server-owned scope и возвращает одно из трёх состояний:

- `available` — обязательные факты и версии источников достаточны; запуск разрешён;
- `no_data` — в выбранном периоде в разрешённом scope действительно нет доменных фактов; run и snapshot не создаются, UI показывает «За период с … по … данных нет»;
- `source_incomplete` — относящиеся к периоду факты есть, но обязательная история, политика, sealed snapshot или другая часть источника неполна; run не создаётся, UI объясняет, какие данные необходимо подготовить.

`no_data` является штатным результатом запроса, а не ошибкой. `partial`, `stale`, `unavailable` и `source_incomplete` не заменяются `no_data`. Organization, project, actor и resource scope берутся только из server auth/context; клиент может лишь сузить уже разрешённый scope и период.
