...
> Актуализация 2026-08-23: этот план исполнен, а затем заменён MS-012. Единственным каноническим профилем является `phpunit.xml` с PostgreSQL; упоминания отдельного `phpunit.postgres.xml` ниже относятся к истории реализации.
