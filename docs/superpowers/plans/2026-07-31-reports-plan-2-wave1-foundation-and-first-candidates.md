...
**Goal:** Создать изолированный контур кандидатов Wave 1 и доказуемые source-backed реализации первых четырёх отчётов, не активируя и не публикуя ни один отчёт.

**Architecture:** Отдельный candidate manifest фиксирует неизменяемую идентичность 12 Wave 1 кандидатов и не участвует в `management-catalog.v1.yaml`, `ReportDefinitionRegistry` или runtime execution. `WaveOneCandidateBindingSet` является проверяемым контрактом порядка и статуса: четыре binding-а получают реальные provider triples, остальные восемь имеют только явный source-contract status. Провайдеры используют существующие Budgeting services через тонкие adapters и передают через существующий `ReportSourceConformanceHarness` курсоры, scope, redaction, fixtures и property evidence.

**Tech Stack:** PHP 8.2, Laravel 11, PHPUnit, Larastan/PHPStan, Symfony YAML, существующие `App\BusinessModules\Core\Reporting` contracts, deterministic PHP fixtures.

## Global Constraints

- Работать только в отдельной ветке, не в `main`; не менять `management-catalog.v1.yaml`, publication ledger, activation services или published registry.
- Новые PHP-файлы используют `declare(strict_types=1);`, PSR-12 и namespace `App\BusinessModules\Core\Reporting`.
- Не запускать миграции, DB CLI, tinker, сидеры, dev server или build; тесты Wave 1 не требуют локальной БД.
- В production-коде не создавать synthetic source data, fake providers или вычисления для кодов без утверждённого source contract.
- `ReportDefinitionBindingAssembler::register()` и `assemble()` остаются единственным published runtime binding protocol; Wave 1 set не регистрируется в нём.
- Любой provider использует actor/organization/project scope из `ReportExecutionContext`; request не задаёт `organization_id` или `owner_id`.
- Для результатов с финансовыми/контрагентскими полями обязателен redaction до сериализации и до conformance fixture digest.
- Кандидатный manifest имеет только publication `candidate`; задача не создаёт active/published activation, HTTP routes или UI.
- Каждая property family даёт 500 детерминированных seeds; четыре реализованные семьи дают минимум 2 000 проверенных seeds.
...
Reject duplicate, missing, reordered or extra codes. Require a non-null `ReportDataProvider` only for the four implemented rows, and require `null` provider for every blocked row. In `docs/reports/wave-1-source-contracts.md` record for G06, G11, G21–G24 the upstream owner, required source fields, grain, scope key, freshness rule and acceptance test; record G12/G13 separately with formula inputs, sign convention, period-close rule and the same acceptance fields. The document calls these rows blocked by an explicit source contract, not executable reports.
...
Run: `git add -- tests/Support/Reporting/WaveOneDeterministicSeedGenerator.php tests/Fixtures/Reporting/WaveOne tests/Unit/Reporting/Conformance/WaveOneCandidateConformanceTest.php; git commit -m "test[reports]: добавлены conformance данные первой волны"`
...
Run: `git add -- docs/reports/wave-1-source-contracts.md docs/reports/wave-1-candidate-handoff.md tests/Architecture/Reporting/WaveOneCandidateIsolationTest.php; git commit -m "docs[reports]: зафиксирована передача первой волны кандидатов"`
