...
- Create: `app/BusinessModules/Core/Reporting/Application/Evidence/PlanOneBGateArtifactRecorder.php`
- Create: `scripts/reporting/run-plan-1b-gate.php`
...
- Create: `tests/Unit/Reporting/Evidence/PlanOneBGateArtifactRecorderTest.php`
...
- Modify: `.gitignore`
- Modify: `.superpowers/sdd/2026-07-26-reports-plan-1b-execution-exports/task-14-report.md`
- Generated after successful CI only; never stage: `build/reports/plan-1b-completion.json`
- Generated by isolated CI gates only; never stage: `build/reports/gates/`
...
- `PlanOneBGateArtifactRecorder::recordPhpUnit(string $gateId, array $processResult, string $resultArtifactPath, ?string $measurementArtifactPath, string $repositoryRevision): array`.
- `PlanOneBGateArtifactRecorder::recordStaticAnalysis(string $resultArtifactPath, string $repositoryRevision): array`.
- CI invokes `php scripts/reporting/run-plan-1b-gate.php <gate-id>`; before and after execution the runner requires the same globally clean current revision, while generated ignored artifacts remain outside porcelain, and it executes argv arrays through Symfony Process without a shell.
...
Run:
```bash
vendor/bin/phpunit tests/Unit/Reporting/Evidence/PlanOneBEvidenceValidatorTest.php tests/Unit/Reporting/Evidence/PlanOneBEvidenceBuilderTest.php tests/Unit/Reporting/Evidence/PlanOneBGateArtifactRecorderTest.php tests/Architecture/Reporting/PlanOneBCrossFileSymbolTest.php tests/Contract/Reporting/PlanOneBEndToEndContractTest.php
```
...
`PlanOneBGateArtifactRecorder` is the single gate catalog and machine-result parser. PHPUnit gates execute the full exact ordered list of relevant existing test paths with `--log-junit build/reports/gates/results/<gate>.junit.xml`; the recorder requires the real zero exit code, strict timestamps/duration, stdout/stderr digests, a well-formed JUnit document, the exact complete suite set, zero errors/failures/skips, and an explicit minimal check-to-suite mapping. Static analysis runs `php -l` separately for every changed production PHP file and one PHPStan invocation for the same exact file set with `--error-format=json`; the runner writes and the recorder reparses a closed typed result artifact. Performance gates additionally run the locked first-party measurement test after successful main PHPUnit: a per-run nonce binds its raw output, and the runner writes a revision-, command-, process- and digest-bound typed measurement artifact which the recorder reparses. Stale or caller-supplied measurement files are removed before execution. Callers never supply `passed` case records or measurement arrays.
Required gates:
```text
plan1a_handoff
ownership_boundary
...
Run targeted unit, architecture, and contract tests from Tasks 1 through 14. Run PostgreSQL, disposable-S3, queue, and performance suites only in their isolated CI jobs. The static-analysis runner must execute PHP syntax checks and PHPStan for every changed Task 14 production PHP file, including the recorder and CI runner itself, and persist the typed machine result before recording the gate.
...
**Step 8: Commit**
```bash
git add docs/reports/contracts/plan-1b-evidence.schema.json app/BusinessModules/Core/Reporting/Application/Evidence scripts/reporting/run-plan-1b-gate.php tests/Unit/Reporting/Evidence tests/Fixtures/Reporting/plan-1b-completion.valid.json tests/Architecture/Reporting/PlanOneBCrossFileSymbolTest.php tests/Contract/Reporting/PlanOneBEndToEndContractTest.php .gitignore .superpowers/sdd/2026-07-26-reports-plan-1b-execution-exports/task-14-report.md docs/superpowers/plans/2026-07-26-reports-plan-1b-execution-exports.md
...
### Task 14 review amendment — round 4

The round 3 caller-provided JSON stdout contract is replaced rather than extended. The first-party `scripts/reporting/run-plan-1b-gate.php` orchestration requires a globally clean worktree before resolving the checked-out revision, rechecks the same clean revision before publication, launches argv arrays through Symfony Process without a shell, captures the actual exit code, UTC timestamps, duration, stdout and stderr, and then delegates to `PlanOneBGateArtifactRecorder`. PHPUnit commands always include the complete exact ordered gate test-path list and `--log-junit build/reports/gates/results/<gate>.junit.xml`. The recorder reparses that file, requires the exact complete suite set with no failures, errors or skips, and derives every typed check record from an explicit locked check-to-suite mapping. Malformed XML, a missing suite and any nonzero process exit fail closed.

Performance gates do not accept an input measurement path. After successful main PHPUnit the runner removes stale measurement outputs and launches the locked first-party measurement test with a fresh random nonce and the exact repository revision. It binds the actual measurement-process command, exit code, timestamps, stdout/stderr and raw-output SHA-256 into a typed artifact; the recorder validates that artifact, its exact measurement IDs/units/limits/statuses and its digest before copying values into gate evidence. The PDF dependency-lock check is backed by an executable assertion for `barryvdh/laravel-dompdf v3.1.1` and `dompdf/dompdf v3.1.4`; every other required check has an explicit non-empty suite subset rather than inheriting an entire gate automatically.

The static-analysis gate executes `php -l` once for each changed production PHP file and one PHPStan process for the same exact file set with `--error-format=json --no-progress` and the required memory limit. The runner writes a closed typed machine-result artifact; the recorder reparses all per-process commands, files, exit codes, timestamps and PHPStan totals before producing `changed_php_syntax` and `changed_php_phpstan`. No CI caller can submit handwritten `passed` records.

`run_export_observability` is bound to `tests/Unit/Reporting/Telemetry/ReportExecutionTelemetryTest.php`. `execution_attempt_leases` is bound to the exact complete 15-path run/export runtime, lifecycle, watchdog, recovery, listener and ABA set. Every other gate is bound to the complete ordered existing test-path set named by its machine artifact, and every required check names the exact executed suite subset that proves it.

Round 4 uses base `d89a4432921148097cc514cdc885c5508d2932cd` and has this exact 13-path commit manifest:

1. `.gitignore`
2. `.superpowers/sdd/2026-07-26-reports-plan-1b-execution-exports/task-14-report.md`
3. `app/BusinessModules/Core/Reporting/Application/Evidence/PlanOneBEvidenceValidator.php`
4. `app/BusinessModules/Core/Reporting/Application/Evidence/PlanOneBGateArtifactRecorder.php`
5. `docs/reports/contracts/plan-1b-evidence.schema.json`
6. `docs/superpowers/plans/2026-07-26-reports-plan-1b-execution-exports.md`
7. `scripts/reporting/run-plan-1b-gate.php`
8. `tests/Architecture/Reporting/PlanOneBCrossFileSymbolTest.php`
9. `tests/Contract/Reporting/PlanOneBEndToEndContractTest.php`
10. `tests/Fixtures/Reporting/plan-1b-completion.valid.json`
11. `tests/Unit/Reporting/Evidence/PlanOneBEvidenceBuilderTest.php`
12. `tests/Unit/Reporting/Evidence/PlanOneBEvidenceValidatorTest.php`
13. `tests/Unit/Reporting/Evidence/PlanOneBGateArtifactRecorderTest.php`

Generated `build/reports/gates/` machine results and `build/reports/plan-1b-completion.json` remain ignored and must not be staged.
