...
- R12 `contract_settlement_exposure` «Расчёты и риски по договорам»: canonical backend и двадцать третья карточка единого admin-каталога опубликованы в обычном контуре управления договорами и не требуют корпоративного модуля нескольких организаций. Организация, actor и разрешённый scope определяются server-owned context; пользователь сужает расчёт только реальными point-in-time options проектов, договоров, распределений, типизированных сторон, направлений, инструментов, статусов, валют и просрочки. Сальдо равно принятому объёму минус оплату, неисполненный и неоплаченный объёмы ограничены снизу нулём, валюты не смешиваются. История owner-сущностей зафиксирована append-only checkpoint, contractor/supplier identity не смешивается при одинаковом ID, а latest-as-of чтение ограничено PostgreSQL индексом. Owner-history PR #237 развёрнут workflow `31054652160`, runtime PR #240 — workflow `31057348975`, identity fix PR #243 — workflow `31060461348`, performance PR #246 — workflow `31061499900`, options PR #247 — workflow `31062988080`; admin PR #38 развёрнут workflow `31083647822` с production release SHA `9d6c13eb638fa99d5f6454b9d2ac088f6dfa0be8`. Production smoke подтвердил 200 для каталога и R12 route, exact `/release.json` и наличие route/report/formula markers в активном asset. Formula fingerprint — `b0c715bcda2e44886ac32fd37e8dc3e30edc333adb01801e5f7f21481d65b9f2`, source fingerprint — `a9003f712e1a298d546cada5dda0232d476157a6e10ae8d418116608d680ba1c`. Полный handoff: `.superpowers/sdd/2026-08-04-reports-architecture-simplification/r12-delivery-report.md`. YouTrack evidence не создан по уже зафиксированной причине недоступности project/issue namespace `180-*`; фиктивный идентификатор не использовался.
- R08 `accepted_production_progress` «Динамика принятой выработки»: canonical backend и двадцать четвёртая карточка единого admin-каталога опубликованы в обычном контуре управления договорами и не требуют корпоративного модуля нескольких организаций. Организация, проект, actor и scope определяются server-owned context; пользователь задаёт дату среза, период и сужает выборку только реальными project-scoped options работ, актов, подрядчиков, единиц, зон и статусов. Принятый объём учитывает приёмки и отмены на их собственные даты, нулевой знаменатель даёт неизвестную долю, единицы и валюты не смешиваются, ставки и стоимость скрыты без sensitive-права. History-boundary PR #259 развёрнут workflow `31088702559`, precision PR #260 — workflow `31096575965`, sensitive-source PR #261 — workflow `31101957096`, readiness PR #262 — workflow `31102872892`, runtime PR #263 — workflow `31109201740`, options PR #264 — workflow `31111774777`; admin PR #40 развёрнут workflow `31117691721` attempt 2 с production release SHA `6438faa401acd0fdbc678f70ef1cbae8f3e572fd`. Production smoke подтвердил 200 для R08 route, стандартный 401 catalog API без сессии, exact `/release.json` и route/report/formula markers в активном asset. Formula fingerprint — `839ea0b2787a0d73872bf5f7a63292437abaae05abb108ae92731abe3264f06b`, source fingerprint — `620c23ba9f377b1b1b0b963a3b8dc39e5c3824920af37f715a429f0203981813`. Полный handoff: `.superpowers/sdd/2026-08-04-reports-architecture-simplification/r08-delivery-report.md`; пользовательская инструкция — YouTrack `180-79` / `Pro-A-68`.
- R01 `project_portfolio_health` «Здоровье портфеля проектов»: в production поставлены fail-closed source readiness и отдельный immutable runtime без преждевременной публикации отчёта. Точный tuple объединяет совместимые owner-снимки `project_margin`, `budget_plan_fact`, `wip_completion_forecast` и выбранный `portfolio_liquidity` calendar на одном server-owned organization/project scope и `as_of`; source rows повторно проверяются и читаются в существующей `REPEATABLE READ` границе. Provider больше не вызывает mutable health-materialization, суммы считаются в minor units отдельно по валютам, пустой server project scope и клиентское расширение scope отклоняются до source access, а реальная persisted shape с пустыми row refs поддерживается без выдуманных domain-ссылок. Опубликованный R04 не получил ложного fingerprint drift: `BudgetingPortfolioProjectionService` сохранён неизменным. Readiness PR #268, merge `48b0cc7267284fb5e2f2b49cb8d3e738df256d53`, workflow `31136817366` — success; runtime PR #270, merge `2b0589e626a8bd2c6ba30c22a55d3975374d0492`, workflow `31139578212` — success. Read-only production evidence подтвердил совпадение SHA-256 пяти ключевых runtime-файлов, нулевое число новых R01 source/selection ошибок в последних 5000 строках лога и стандартный 401 catalog API без сессии. Миграций, новых индексов, config, feature flags, очередей и workflow не добавлено. R01 остаётся скрытым до отдельных options/publication/admin UI/DoD блоков, общий итог остаётся 24/28. Evidence: `.superpowers/sdd/2026-08-04-reports-architecture-simplification/r01-readiness-foundation-evidence.md`, `.superpowers/sdd/2026-08-04-reports-architecture-simplification/r01-immutable-runtime-evidence.md`.
