...
- R01 `project_portfolio_health` «Здоровье портфеля проектов»: в production поставлен только fail-closed фундамент source readiness без преждевременной публикации отчёта. Точный tuple требует совместимые неизменяемые owner-снимки `project_margin`, `budget_plan_fact`, `wip_completion_forecast` и доказательство `portfolio_liquidity` на одном server-owned organization/project scope и `as_of`; проверяются definition/query identity, parent EPM snapshot, версии, закрытие периода, полнота строк, валюты и когорта. Completeness lookup использует существующий unique-index, миграций и новой инфраструктуры нет. Backend PR #268, merge `48b0cc7267284fb5e2f2b49cb8d3e738df256d53`, production workflow `31136817366` — success; read-only production evidence подтвердил exact SHA, наличие новых классов и стандартный 401 catalog API без сессии. Последняя доступная запись production-лога предшествовала deploy, поэтому отсутствие ошибок новой версии по логам пока не заявляется. `assertImmutableHealthCoverage()` не снят, R01 остаётся скрытым, общий итог остаётся 24/28. Evidence: `.superpowers/sdd/2026-08-04-reports-architecture-simplification/r01-readiness-foundation-evidence.md`.
