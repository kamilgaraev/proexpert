...
9. Остальные отчёты — по одному, в порядке реальной готовности источников. Перед каждой реализацией выполняется Definition of Ready audit; порядковый номер в каталоге сам по себе не является основанием публиковать отчёт.
