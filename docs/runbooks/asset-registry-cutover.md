...
Каждая JSON-запись также содержит `observed_at` и `release_sha`. Поэтому непрерывность окна подтверждается по UTC-меткам, а все почасовые результаты можно привязать к конкретному immutable release. Пустой `release_sha` в production считается недостаточным доказательством и запрещает переход к Фазе B.

Для read-only разбивки причин используется `--details`. Она включает обезличенный блок `activity` за последние `ASSET_REGISTRY_OBSERVATION_HOURS`; точное начало окна можно передать через `--since=<ISO-8601>`. `completed_operational_cycles` учитывает только упорядоченную цепочку одной canonical-единицы: приёмка, назначение/выдача, сменный рапорт и его утверждение внутри окна. Некорректный или будущий `--since` завершает команду с ошибкой.

Перед фактическим запуском обязателен чистый `assets:backfill --dry-run --format=json`. Затем штатный deployment выполняет идемпотентный `assets:backfill`: он создаёт отсутствующие canonical-пары, переносит ссылку во все таблицы операций и согласует размещение с фактически действующим назначением. Если два legacy-назначения пересекаются, более позднее назначение считается заменой предыдущего, а предыдущий интервал закрывается точно на границе нового. Записи не удаляются. Одинаковое время старта или недоступный организации проект считаются неоднозначным конфликтом: команда завершается с ошибкой до изменения этой карточки. Проект, принадлежащий другой организации, допустим только при активном участии организации-владельца актива через `project_organization`.
...
6. Зафиксировать для начала и конца интервала: release SHA, UTC-время, JSON команды, число scheduler-запусков и отсутствие ошибок записи. В конечном `--details` результате требуется `completed_operational_cycles >= 1`.
