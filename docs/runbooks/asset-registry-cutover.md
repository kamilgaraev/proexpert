...
Для read-only разбивки причин используется `--details`. Она включает обезличенный блок `activity` за последние `ASSET_REGISTRY_OBSERVATION_HOURS`; точное начало окна можно передать через `--since=<ISO-8601>`. `completed_operational_cycles` учитывает только упорядоченную цепочку одной canonical-единицы: приёмка, назначение/выдача, сменный рапорт и его утверждение внутри окна. Утверждение доказывается одновременно статусом `approved` и заполненным `approved_at`; одна временная метка при противоречивом статусе не засчитывается. Некорректный или будущий `--since` завершает команду с ошибкой.
