...
- `001200_close_truthful_settings_schema` переводит новые settings snapshots на точную схему v2,
  исключает старые v1 snapshots из admin/runtime-выборки и обновляет pin-функцию. Исторические
  v1 snapshots не переписываются и не удаляются; после миграции до создания первого глобального
  v2 snapshot AI-сметчик намеренно закрыт для новых операций. Миграция forward-only, schema rollback запрещён.
