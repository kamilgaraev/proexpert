...
| `POST /sets/{set}/compose-act` | Составить черновик по одной из пяти форм актов; JSON с `document_type`, `title`, `profile_data`, связями и участниками |

Пункт перечня содержит `key`, `profile_type`, `title`, необязательные `stage`, `work_type_id`, `completed_work_id`, `project_location_id`. Для пяти шаблонных актов требуется `conditions` с булевыми `designer_supervision` и `separate_executor`; их устанавливают при сверке с утверждённым файлом и проектом. Ответ перечня содержит `revision`, `file_hash` (SHA-256), дату, сторону и исходное имя файла. `approved_list_id` возвращается с комплектом. Смена перечня создаёт новую запись; уже переданный комплект хранит прежний снимок в манифесте.

Существующий `POST /sets/{set}/documents` принимает прежний `initial_version[file]` или `source_warehouse_passport_file_id` для `quality_passport`. Для складского источника `initial_version` можно не передавать: номер первой редакции будет `1.0`. Одновременно оба источника не допускаются. Складской файл копируется в частное хранилище ИД; метаданные редакции содержат идентификаторы прихода и источника, а также SHA-256 исходника. Ограничение организации и объекта проверяется сервером.
...
`compose-act` принимает только пять предусмотренных видов актов. Поля формы, обязательные связи и сведения об участниках акта с основаниями полномочий валидируются до генерации PDF; дополнительные участники зависят от условий пункта перечня. Результатом становится черновик с редакцией `generated_preparation`. Сгенерированный PDF не считается подписанным актом.
