...
- Admin: `../prohelper_admin/.github/workflows/deploy.yml`, production build с `VITE_API_URL=https://api.1мост.рф`.
- Marketing/LK: `../prohelper_land/.github/workflows/deploy.yml`, production paths `/var/www/prohelper_marketing`, `/var/www/prohelper_lk`, API `https://api.1мост.рф`.
...
Поиск по всему workspace нашел `https://dev.1мост.рф` только в старой документации по сметам/frontend, без связанного deploy workflow или RAG runbook. Публичные проверки `2026-05-23 22:48 +03:00` завершились таймаутом для `https://dev.1мост.рф`, `https://api.1мост.рф/up` и `https://admin.1мост.рф`; runtime evidence не собран.
