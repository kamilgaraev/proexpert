...
Документ фиксирует целевую архитектуру CRM-домена для МОСТ: компании, контакты, лиды, сделки, активности, документы и связи с существующими строительными сущностями. Спецификация основана на текущем коде `prohelper` и `prohelper_admin` и нужна как вход для последующей реализации backend/API/admin UI.
...
МОСТ в CRM является операционным source of truth для presale, коммуникаций, pipeline, дедупликации, связей и управленческих коммерческих оценок.
...
- явно зафиксированы границы МОСТ/1С;
